...
using UnityEngine;
    ...
    public sealed class 探索服务
    {
        ...
        // 生成资源/选择效果的获得摘要（物品·金币·经验），用于探索日志播报
        private string 获得摘要(剧情效果 效果)
        {
            ...
            if (效果.金币 > 0) 段.Add(货币工具.文本(效果.金币));
        ...
        private void 遭遇(string 敌人组, int 先手)
        {
            ...
            {
                显示($"你发现了{名称}，它没有察觉到你。", default, new 探索按钮数据("偷袭", "战斗"), new 探索按钮数据("绕开", "绕开"));
                return;
            }
            // 遇见
            显示($"你遇上了{名称}！", default, new 探索按钮数据("战斗", "战斗"), new 探索按钮数据("落荒而逃", "逃跑"));
        }
        ...
        // 普通层主按钮：精力>0 才可探索（返回统一走侧边栏取消）
        private 探索按钮数据 探索按钮() => 档案.精力 > 0 ? new 探索按钮数据("探索", "搜索") : default;
