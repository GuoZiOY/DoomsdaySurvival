...
public sealed class 日常任务服务
{
    ...
    // 结算：发奖并派发事件（末日以物易物：奖励物资 + 经验）
    private void 结算(日常任务 日常)
    {
        日常.进度 = 日常.目标数量;
        日常.已领取 = true;
        int 奖励价值 = 日常.奖励金币 > 0 ? 日常.奖励金币 : 0;
        int 面包数 = Mathf.Max(1, 奖励价值 / 2 + 1);
        档案.添加物品("面包", 面包数);
        事件.发布(new 背包变化事件("面包", 面包数, 变化原因.获得));
        bool 升级了 = 档案.获得经验(日常.奖励经验);
        事件.发布(new 任务完成事件(日常.标识, $"{日常.名称} 完成！+面包×{面包数}、{日常.奖励经验}经验"));
        事件.发布(new 日志事件(日志类型.任务, $"{日常.名称} 完成！获得 面包×{面包数}、{日常.奖励经验} 经验"));
