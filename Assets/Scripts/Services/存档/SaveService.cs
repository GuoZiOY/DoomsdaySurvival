...
public sealed class SaveService
{
    ...
    // 最新的那个可读槽：优先手动槽，都没有才退到自动槽；一个都没有 → -1。
    // **私有**：对外只给 有任何可读存档()（主菜单「继续」的可用性用它）——
    // 公开一个"只有内部用"的查询会变成门禁眼里的死 API（Tools/存档接线核对 段[5] 抓到过两次）。
    // 于是这里也定下了主菜单「继续」的口径：**自动存档是"防丢"的兜底，不该抢在玩家自己选的档前面**；
    // 但"只有自动档时"（玩家从没手动存过）也让他能继续 —— 否则自动存档等于白存。
    private int 最近可读槽()
    {
        int 槽 = 最近手动槽();
    ...
    private 存档数据 组装(玩家档案 玩家, bool 自动, string 槽位名)
    {
        ...
        // ★ 刀65：战局快照 —— "你在哪一层、哪一格、周围翻过什么、敌人在哪"。
        // 抓不到（没装配）→ null，读档走"从安全屋出发"那条老路。
        var 战局 = ServiceRegistry.已注册<大世界探索服务>() ? 战局存档.取() : null;
        return new 存档数据
        {
            头 = new 存档头
            {
                ...
                游戏分钟数 = 玩家.游戏分钟数,
                // ★ 刀65：位置改成真的（原来恒为"安全屋"）。老档没有 战局 → 回落"安全屋"。
                位置 = 战局 != null ? 战局.位置文本() : "安全屋",
                自动 = 自动,
            },
            玩家 = 玩家,
            整点基准 = 取整点基准(),
