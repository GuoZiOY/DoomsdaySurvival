...
public abstract class 格子探索服务
{
    ...
    // ★ 刀61：玩家每走完一步的通知（派生：大世界用它驱动敌人 AI + 发出脚步声）
    protected virtual void 玩家走了一步() { }
    // ★ 刀61：本层对噪音的反应。派生覆写（大世界 = 让敌人听见并改变状态）；
    //   房间层/区域层的敌人这一刀还是**静态**的（见 docs/大世界敌人AI设计.md §2），所以它们不覆写。
    protected virtual void 噪音结算(噪音事件 噪) { }
    ...
    // 传播范围：**同层**（大世界/区域/房间各算各的）。跨层（房间里开枪吵到街上）留到后面做，
    //   它需要"楼层 → 区域内建筑入口格"的映射。
    // 注：这一刀只有大世界层的敌人会动（房间/区域仍是静态撒点），所以只有 大世界探索服务 覆写 噪音结算；
    //     另外两层调 播噪音 是**没有副作用的空操作** —— 所以那两层这一刀干脆不调（不留悬空线）。
    ...
    // 玩家这一下攻击有多响（按**当前持用的武器种类**）：枪械 90 / 弓弩 30 / 近战 25。
