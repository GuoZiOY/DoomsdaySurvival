...
using UnityEngine;
    ...
    public sealed class PlayerService
    {
        ...
        // 把背包解析器接到 背包服务（形状/堆叠上限/有效最大耐久/重量）
        private void 接线背包服务(玩家档案 档案)
        {
            var 背 = 档案.背包服务;
            背.形状解析 = 档案.形状解析;
            背.堆叠上限解析 = 档案.堆叠上限解析;
        ...
        private void 发布初始状态()
        {
            ...
            事件.发布(new 魔力变化事件(档案.行动点, 档案.最大行动点, 0));
        ...
        private static void 清理非法值(玩家档案 档案)
        {
            ...
            档案.背包服务.背包 ??= new List<物品堆叠>();   // 背包列表由 背包服务 持有
            ...
            // 迁移：旧存档中 列<0 的未入格堆叠 → 尝试放入网格（背包满的保留原状，面板不显示）
