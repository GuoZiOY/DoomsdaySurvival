using System.Collections.Generic;
using UnityEngine;

// 商店设施：购买/卖出 业务规则（UI 无关，反馈走事件）
public sealed class 商店设施 : 设施逻辑
{
    // 可购买商品：价格>0 且非任务物品
    public 物品数据[] 可购买商品()
    {
        var 列表 = new List<物品数据>();
        foreach (var 物品 in 数据.物品.Values)
            if (物品.价格 > 0 && 物品.类型 != "任务") 列表.Add(物品);
        return 列表.ToArray();
    }

    // 尝试购买：扣金币，武器/防具直接装备，其余入包
    public bool 尝试购买(string 物品标识)
    {
        if (!数据.物品.TryGetValue(物品标识, out var 物品)) return false;
        if (玩家.金币 < 物品.价格) { 事件.发布(new 日志事件(日志类型.反馈坏, $"金币不足（需要 {物品.价格}）。")); return false; }
        玩家.金币 -= 物品.价格;
        if (物品.类型 == "武器") 玩家.武器标识 = 物品.标识;
        else if (物品.类型 == "防具") 玩家.防具标识 = 物品.标识;
        else 玩家.添加物品(物品.标识);
        事件.发布(new 金币变化事件(玩家.金币, -物品.价格));
        事件.发布(new 日志事件(日志类型.反馈, $"购买 {物品.名称}（-{物品.价格} 金币）"));
        return true;
    }

    // 可卖出物品：背包里 价格>0 的堆叠
    public 物品堆叠[] 可卖出物品()
    {
        var 列表 = new List<物品堆叠>();
        foreach (var 堆叠 in 玩家.背包)
            if (数据.物品.TryGetValue(堆叠.标识, out var 物品) && 物品.价格 > 0) 列表.Add(堆叠);
        return 列表.ToArray();
    }

    // 尝试卖出：按半价回收（最低 1 金）
    public bool 尝试卖出(string 物品标识)
    {
        if (!数据.物品.TryGetValue(物品标识, out var 物品) || 物品.价格 <= 0) return false;
        if (!玩家.移除物品(物品标识)) { 事件.发布(new 日志事件(日志类型.反馈坏, "你没有这件物品。")); return false; }
        int 价 = Mathf.Max(1, 物品.价格 / 2);
        玩家.金币 += 价;
        事件.发布(new 金币变化事件(玩家.金币, 价));
        事件.发布(new 日志事件(日志类型.反馈, $"卖出 {物品.名称}（+{价} 金币）"));
        return true;
    }
}
