...
public sealed class 训练逻辑
{
    ...
    // 尝试属性训练：花铜币增强对应基础属性（不消耗自由属性点）；花费单位 = 铜币
    public bool 尝试训练(string 项目标识)
    {
        if (!数据.训练项目.TryGetValue(项目标识, out var 项目)) return false;
        if (玩家.铜币 < 项目.花费) { 音效管理器.实例?.播放失败(); 事件.发布(new 日志事件(日志类型.反馈坏, $"铜币不足（需要 {货币工具.文本(项目.花费)}）。")); return false; }
        玩家.铜币 -= 项目.花费;
        玩家.训练属性(解析属性(项目.属性), 项目.加成);
        事件.发布(new 金币变化事件(玩家.铜币, -项目.花费));
        事件.发布(new 属性变化事件(玩家.体力, 玩家.力量, 玩家.智力, 玩家.敏捷, 玩家.自由属性点));
        事件.发布(new 日志事件(日志类型.反馈, $"完成{项目.名称}！{项目.属性}+{项目.加成}（-{货币工具.文本(项目.花费)}）"));
        return true;
    }
    ...
    // 熟练训练花费：按当前熟练等级 30×N 金（N=当前等级；换算铜币）
    public int 熟练训练花费(string 技能标识) => 货币工具.组合(30, 0, 0) * Mathf.Max(1, 玩家.技能熟练等级(技能标识));
    ...
    public bool 尝试熟练训练(string 技能标识)
    {
        ...
        int 花费 = 熟练训练花费(技能标识);
        if (玩家.铜币 < 花费) { 音效管理器.实例?.播放失败(); 事件.发布(new 日志事件(日志类型.反馈坏, $"铜币不足（需要 {货币工具.文本(花费)}）。")); return false; }
        玩家.铜币 -= 花费;
        int 新等级 = ServiceRegistry.Get<技能服务>().记录熟练度(技能标识, 熟练训练点数);
        事件.发布(new 金币变化事件(玩家.铜币, -花费));
        事件.发布(new 日志事件(日志类型.反馈, $"{技能.名称} 熟练度+{熟练训练点数}（{玩家.技能熟练度(技能标识)}/级），熟练等级 {新等级}（-{货币工具.文本(花费)}）"));
        return true;
    }
