...
public sealed class 训练逻辑
{
    ...
    public bool 尝试训练(string 项目标识)
    {
        ...
        if (玩家.金币 < 项目.花费) { 音效管理器.实例?.播放失败(); 事件.发布(new 日志事件(日志类型.反馈坏, $"金币不足（需要 {项目.花费}）。")); return false; }
        玩家.金币 -= 项目.花费;
    ...
    public bool 尝试熟练训练(string 技能标识)
    {
        ...
        int 花费 = 熟练训练花费(技能标识);
        if (玩家.金币 < 花费) { 音效管理器.实例?.播放失败(); 事件.发布(new 日志事件(日志类型.反馈坏, $"金币不足（需要 {花费}）。")); return false; }
        玩家.金币 -= 花费;
