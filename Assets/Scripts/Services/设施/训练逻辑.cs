...
public sealed class 训练逻辑
{
    ...
    public bool 尝试训练(string 项目标识)
    {
        ...
        if (玩家.行动点 < 训练行动点) { 音效管理器.实例?.播放失败(); 事件.发布(new 日志事件(日志类型.警告, "行动点不足，先休息一下。")); return false; }
        if (!玩家.持有物品(训练食物)) { 音效管理器.实例?.播放失败(); 事件.发布(new 日志事件(日志类型.警告, $"训练需要 {食物名()} 补充体力。")); return false; }
        玩家.消耗行动点(训练行动点);
        玩家.移除物品(训练食物);
        ...
        事件.发布(new 日志事件(日志类型.角色, $"完成{项目.名称}！{项目.属性}+{项目.加成}（耗 {训练行动点} 行动点 + 1 {食物名()}）"));
        return true;
    }
    ...
    public bool 尝试熟练训练(string 技能标识)
    {
        ...
        if (玩家.行动点 < 熟练训练行动点) { 音效管理器.实例?.播放失败(); 事件.发布(new 日志事件(日志类型.警告, "行动点不足，先休息一下。")); return false; }
        玩家.消耗行动点(熟练训练行动点);
        int 新等级 = ServiceRegistry.Get<技能服务>().记录熟练度(技能标识, 熟练训练点数);
        事件.发布(new 精力变化事件(玩家.行动点, 玩家.最大行动点, -熟练训练行动点));
        事件.发布(new 日志事件(日志类型.角色, $"{技能.名称} 熟练度+{熟练训练点数}（{玩家.技能熟练度(技能标识)}/级），熟练等级 {新等级}"));
        return true;
    }
