...
using UnityEngine;
    ...
    public sealed class PlayerService
    {
        ...
        private void 接线解析器(玩家档案 档案)
        {
            ...
            档案.技能解析 = 标识 => 数据.技能.TryGetValue(标识, out var 技) ? 技 : null;   // 被动技能：标识 -> 技能数据（战斗投影 分流 + 增幅）
            档案.知识等级解析 = 标识 => 数据.物品.TryGetValue(标识, out var 书) ? 书.知识等级 : null;   // 知识书：书籍标识 -> 知识等级表（知识加成 聚合）
        ...
        // 应用职业：属性分布（直接给定五维，覆盖初始 5）+ 初始技能（多条）+ 初始知识 + 初始配方
        //          + 随机书 + 初始装备 + 职业天赋
        private void 应用职业(string 职业标识)
        {
            ...
            // 随机书放在**装备之后**发：这时背包/弹挂已经穿在身上，书会落进书包（放前面的话统一放入只能进仓库）
            发随机书(职业标识, 职业.随机配方书级别, "配方书");
            发随机书(职业标识, 职业.随机技能书级别, "技能书");
            if (!string.IsNullOrEmpty(职业.天赋) && !档案.天赋.Contains(职业.天赋))
                档案.天赋.Add(职业.天赋);
        }

        // 初始技能（多条；冲刺/后撤 + 职业专属）：**检查 `学习技能` 的返回值**。
        //   为什么必须检查：`成长管理器.学习技能` 在"属性前提不满足"时返回 false 且什么都不做，
        //   旧写法 `档案.学习技能(技能);` 把返回值扔了 → 表现是"这个职业开局少一招"，而且一条日志都没有。
        private void 学初始技能(string 职业标识, 职业数据 职业)
        {
            if (职业.初始技能 == null) return;
            foreach (var 标识 in 职业.初始技能)
            {
                if (string.IsNullOrEmpty(标识)) continue;
                if (!数据.技能.TryGetValue(标识, out var 技能))
                {
                    事件.发布(new 日志事件(日志类型.警告, $"职业[{职业标识}] 的初始技能[{标识}] 不在 skills.json —— 这一招没学到。"));
                    continue;
                }
                if (!档案.学习技能(技能))
                    事件.发布(new 日志事件(日志类型.警告, $"职业[{职业标识}] 的初始技能[{标识}] 没学会：{档案.技能前提失败原因(技能)}"));
            }
        }

        // 初始知识（写的是知识书标识，开局直接给到 **1 级**）：
        //   走 `档案.加知识经验(标识, 0, 门槛, 等级上限)` —— 首次调用即解锁到 1 级（`成长管理器.加知识经验` 的第一条分支）。
        //   ⚠ 门槛**必须从书本体取**：传空表时那条分支会直接 `return (0,0)`，什么都发不出去（静默失效）。
        private void 学初始知识(string 职业标识, 职业数据 职业)
        {
            if (职业.初始知识 == null) return;
            foreach (var 标识 in 职业.初始知识)
            {
                if (string.IsNullOrEmpty(标识)) continue;
                if (!数据.物品.TryGetValue(标识, out var 书) || 书.知识等级 == null || 书.知识等级.Length == 0)
                {
                    事件.发布(new 日志事件(日志类型.警告, $"职业[{职业标识}] 的初始知识[{标识}] 不是知识书（或没配知识等级表）—— 发不下去。"));
                    continue;
                }
                档案.加知识经验(标识, 0, 书.知识门槛, 书.知识等级.Length);
            }
        }

        // 初始配方（手册里的家传手艺）：直接记成**已习得**（不用先读那本书）
        private void 记初始配方(string 职业标识, 职业数据 职业)
        {
            if (职业.初始配方 == null) return;
            foreach (var 标识 in 职业.初始配方)
            {
                if (string.IsNullOrEmpty(标识)) continue;
                if (!数据.配方.ContainsKey(标识))
                {
                    事件.发布(new 日志事件(日志类型.警告, $"职业[{职业标识}] 的初始配方[{标识}] 不在配方表里 —— 这条配方没记上。"));
                    continue;
                }
                档案.记录习得配方(标识);
            }
        ...
        // 随机书（大学生）：按 `书籍种类` + `书籍级别` 从数据表里随机抽一本塞进行囊。
        //   级别空 = 这个职业不发书；一本都抽不到（数据没配那种书）→ 出声，别静默。
        private void 发随机书(string 职业标识, string 级别, string 种类)
        {
            if (string.IsNullOrEmpty(级别) || string.IsNullOrEmpty(种类)) return;
            var 候选 = new List<string>();
            foreach (var (标识, 物品) in 数据.物品)
                if (物品 != null && 物品.类型 == "书籍" && 物品.书籍种类 == 种类 && 物品.书籍级别 == 级别)
                    候选.Add(标识);
            if (候选.Count == 0)
            {
                事件.发布(new 日志事件(日志类型.警告, $"职业[{职业标识}] 要随机发一本「{级别}」{种类}，但数据里一本都没有 —— 跳过。"));
                return;
            }
            string 选 = 候选[UnityEngine.Random.Range(0, 候选.Count)];
            档案.添加物品(选, 1);
            if (档案.物品数量(选) <= 0)   // 行囊塞不下的兜底：出声（别让玩家以为拿到了）
                事件.发布(new 日志事件(日志类型.警告, $"职业[{职业标识}] 的随机书[{选}] 没放进行囊（容器满？）。"));
        }
