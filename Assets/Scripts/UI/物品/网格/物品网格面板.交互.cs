...
public partial class 物品网格面板
{
    ...
    private void 物品打开拆分()
    {
        if (选中 == null) { Debug.LogWarning("[背包] 拆分：未选中物品（右键目标面板未匹配）。"); return; }
        if (选中.数量 <= 1) { ServiceRegistry.Get<EventBus>()?.发布(new 日志事件(日志类型.警告, $"拆分：{选中.标识} 数量 {选中.数量} ≤ 1，不可拆分。")); return; }
        var 面板 = 拆分面板.实例;
        if (面板 == null)
        {
        ...
        if (面板 == null)
        {
            Debug.LogWarning("[背包] 场景未搭建 拆分面板（拆分面板 组件 + 面板根）。");
            return;
        }
        面板.打开(选中, this);
    }
    ...
    private void 物品拆分(int 数量)
    {
        ...
        if (容器视图)
        {
            var 新堆叠 = new 物品堆叠(选中.标识, 数量) { 旋转 = 选中.旋转, 当前耐久 = 选中.当前耐久, 品质 = 选中.品质, 已装填 = 选中.已装填, 配件 = 选中.配件 != null ? new List<配件条>(选中.配件) : null };
            选中.数量 -= 数量;
            int 实放 = 档案.放入堆叠(新堆叠);
            if (实放 > 0)
            {
                ...
                选中 = null;
                请求刷新();
                事件?.发布(new 日志事件(日志类型.获得, $"容器已满，拆分出的 {新堆叠.标识}×{数量} 放入穿戴容器/仓库。"));
        ...
        音效管理器.实例?.播放失败();
        事件?.发布(new 日志事件(日志类型.警告, "拆分失败：容器与 穿戴容器/仓库 均已满，无空位放置拆出物品。"));
    }
    ...
    // 双击快捷操作：恢复品=使用；装备类=换装；容器=打开；书籍=打开书籍面板（阅读走面板里的进度与成功率）
    private void 快捷操作(物品堆叠 堆叠)
    {
        ...
        // 配件（改装件，v51 刀37）：**不能在背包里直接双击装**（用户定的流程：改装在工作台做工具活）——
        //   它会走到下面那条 `槽位 非空 → 装备选中()`，而配件的 槽位 是"枪口/瞄具"这种**配件槽**，
        //   不是装备槽位 → 结果只会是"点了没反应/放失败"。这里明确给一句指引，别让玩家自己猜。
        if (物品.类型 == "配件")
        {
            音效管理器.实例?.播放失败();
            ServiceRegistry.Get<EventBus>()?.发布(new 日志事件(日志类型.警告, $"{堆叠.标识} 是改装件：到【工作台】把装备和它一起放进材料区，点〔装配改装件〕装上。"));
            return;
        }
        if (!string.IsNullOrEmpty(物品.槽位)) { 装备选中(); return; }
        // ⚠ v51 刀15 删掉了一行死分支：原来是 `if (物品.类型 == "技能书") 面板操作.学习技能书(...)`，
        //   而**全 JSON 的类型取值里没有"技能书"这个值**（技能书是 `书籍种类` 的取值之一，
        //   类型只有"书籍"）→ 那行永不执行，双击技能书会掉到"播放失败音"。
        //   正确路径就是下面这行：**所有书都开书籍面板**，由书籍面板按 `书籍种类` 分流
        //   （技能书/配方书/蓝图 各有阅读流程与成功率，直接"学习"会绕过整套阅读机制）。
