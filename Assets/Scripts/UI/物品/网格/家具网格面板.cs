...
public sealed class 家具网格面板 : 网格面板基类
{
    ...
    // 容器型 家具（冰箱/水箱 等：家具数据.是容器）→ 浮动 容器面板（复用 弹药箱 同款）
    public override void 菜单打开()
    {
        if (选中 == null) return;
        var (定义标识, _) = 家具工具.解码(选中.标识);
        // 打开 家具 → 日志（[角色]）；浮动面板（制作/净水器/种植/容器）由 浮动面板基类.创建 统一发，这里补 非浮动 的 储物箱
        if (定义标识 == "储物箱")
        {
            ServiceRegistry.Get<EventBus>()?.发布(new 日志事件(日志类型.角色, $"打开了 {家具名称(选中.标识)}。"));
            面板管理器.实例?.显示面板类型<持有面板>();
            return;
        }
        // 制作 家具（工作台/灶台/医疗站）→ 浮动 制作面板（绑定 家具 实例；Canvas 顶层，可与 背包 共存）
        if (定义标识 == "工作台" || 定义标识 == "灶台" || 定义标识 == "医疗站")
        {
            音效管理器.实例?.播放成功();
            var 挂载父 = transform as RectTransform;
            制作面板.创建(挂载父, 选中);
            return;
        }
        // 容器型 家具：分区容器（净水器 等）→ 净水器面板（3 区 网格 + 净化）；
        // 就地转化 家具（种植箱 播种 / 晾晒架 晾晒）→ 各自 面板（网格 + 进度；两件 网格 尺寸 不同，各自 一个 预制体）；
        // 普通 容器（冰箱/弹药箱 等）→ 浮动 容器面板（单 网格）
        var 容器服务 = ServiceRegistry.Get<容器服务>();
        if (容器服务.是容器(选中))
        {
            容器服务.初始化容器(选中);   // 确保 内部 网格/分区 子容器 存在
            音效管理器.实例?.播放成功();
            var 挂载父 = transform as RectTransform;
            if (容器服务.是分区容器(选中))
                净水器面板.创建(挂载父, 选中);
    ...
    public override void 家具拆除()
    {
        ...
        var 拆 = 选中;
        // 容器型 家具（冰箱 等）内部 有 物品 → 先 清空 才能 拆（防 内容 丢失）
        if (拆.容器物品 != null && 拆.容器物品.Count > 0)
        {
            ServiceRegistry.Get<EventBus>().发布(new 日志事件(日志类型.警告, $"{家具名称(拆.标识)} 里还有东西，先取出来再拆。"));
            音效管理器.实例?.播放失败();
            return;
        }
        选中 = null;
        ...
        请求刷新();
        音效管理器.实例?.播放成功();
        ServiceRegistry.Get<EventBus>().发布(new 日志事件(日志类型.获得, $"拆除了 {家具名称(拆.标识)}。"));
    }
