...
public sealed class 持有面板 : 面板基类
{
    ...
    protected override void 刷新(object 上下文)
    {
        ...
        {
            // 右区怎么显示，看**在不在家**（用户 2026-09-15 定的口径）：
            //   在安全屋：显示仓库（家底）
            ...
            //
            // ⚠ 这个判据的历史（两轮，都记在 项目现状.md §六 #59）：
            //   ① 原来 `外出中` 是坏的 —— 回安全屋**不清 `当前世界`**（只清这一趟的临时数据），
            //      于是人在家里 `大世界探索服务.探索中` 恒为 true → 仓库永远不显示（用户实测报的 bug）。
            //   ② 先打过一个补丁：改问"我盖住的那一层是不是安全屋面板"（`面板管理器.上一个面板`）。
            //   ③ 刀69 把**根**修了：回安全屋会把三层的世界都收掉（格子探索服务.回安全屋收尾）
            //      → `探索中` 恢复成它本来该有的意思（"我在外面"）→ 这里改回按**世界状态**判。
            //      一条口径一个判据：存档记"层"（战局存档.取）与这里读的是同一个事实，不该各绕一套。
            //      （顺带：按世界状态判还更稳 —— 剧情对话这类面板盖上来时，面板槽会被占用，而世界状态不会。）
            bool 在外面 = 外出中;
            if (仓库 != null)
            {
    ...
    // 外出中？（三个探索层任一活着 = 不在安全屋）
    //   三层都收的时机 = 回安全屋（见 格子探索服务.回安全屋收尾）——所以"都不活着"才等于"在家"。
