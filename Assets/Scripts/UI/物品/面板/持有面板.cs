...
public sealed class 持有面板 : 面板基类
{
    ...
    // 注：原来这里有一颗「返回」按钮 —— 用户 2026-09-15 去掉了（关闭交给 HUD 上那颗统一按钮）。
    //   回退() 保留：HUD 的关闭按钮就是调它。场景里那颗按钮物体请自行删掉。
    void Awake()
    {
    ...
    protected override void 刷新(object 上下文)
    {
        ...
        {
            // ★ 用户 2026-09-16 修：右区怎么显示，看**本面板 盖在 哪一层 上**：
            //     盖在 安全屋面板 上 → 显示仓库（家底）
            //     盖在 探索层（大世界/区域/房间）上 → **两个都不显示**（外面的家底不该随手可整理）
            //   搜索面板**只在"搜容器"时才出现**（见上面 `上下文 is 搜索容器` 那条分支）——
            //   自己主动打开背包（F1 / 侧边栏 / 右键储物箱）永远不进搜索模式，所以这里一律收起搜索。
            //   注：装备区 / 装具区 **一直显示** —— 本面板只切右区这个模式位，从不碰它们。
            //
            //   ⚠ 原来判的是"有没有探索层活着"（`外出中` = 大世界/区域/房间 任一 `探索中`）——**那条判据是坏的**：
            //     回安全屋（走到营地门口格 / 大世界面板的「回安全屋」）只发 `打开营地事件` → 各层 `回安全屋清战局()`
            //     清的是"这一趟的临时数据"（临时建筑/搜索计数/视野记忆/敌人 AI 态），**不清 `当前世界`**
            //     （大世界那层照样活着，下次 出门() 才用 `打开默认世界()` 重新生成）。
            //     于是**人在家里时 `大世界探索服务.探索中` 恒为 true** → 判成"在外面" → 仓库永远不显示。
            //     症状就是用户报的这个：在安全屋右键储物箱 / 按 F1 打开背包，右区空着，看不到仓库。
            //   新判据用 `面板管理器.上一个面板`：它由 `显示()` 在切面板那一刻写，答案正是"我盖住的那一层是谁"。
            //     **本面板是盖上去的一层**（底下的面板此时已被隐藏）→ 问这个槽才是对的时机。
            //     （对比 §六 #55 那条坑：那里错在"拿 1 层深的槽当**导航栈**用"（要回溯好几步）；这里只问"我底下是谁"，不回溯。）
            bool 在安全屋 = 面板管理器.实例 != null && 面板管理器.实例.上一个面板 is 安全屋面板;
            if (仓库 != null)
            {
                仓库.gameObject.SetActive(在安全屋);
                if (在安全屋) 仓库.刷新();
            }
            if (搜索 != null)
            {
