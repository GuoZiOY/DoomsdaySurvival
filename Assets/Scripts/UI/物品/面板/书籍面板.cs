...
public sealed class 书籍面板 : 浮动面板基类
{
    ...
    // 知识书 状态行：已解锁等级 / 上限 + 当前级内经验（未解锁 → 提示首次阅读会解锁）
    private string 知识状态文本(书籍服务 服务)
    {
        int 上限 = 书籍.知识等级?.Length ?? 0;
        var 档案 = ServiceRegistry.Get<PlayerService>()?.档案;
        int 级 = 档案?.知识等级(书籍.标识) ?? 0;
        string 率 = $"成功率：{Mathf.RoundToInt(服务.成功率(书籍) * 100)}%";
        if (级 <= 0) return $"知识书　未入门（共 {上限} 级）　首次阅读将解锁　{率}";
        int 经验 = 档案?.查知识(书籍.标识)?.经验 ?? 0;
        return 级 >= 上限
            ? $"知识书　已满级（{级} / {上限}）　{率}"
            : $"知识书　{级} / {上限} 级　经验 {经验} / {玩家档案.知识每级经验}　{率}";
    }
