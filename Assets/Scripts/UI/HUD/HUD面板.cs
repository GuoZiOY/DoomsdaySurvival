...
// 形态：纯文字。生命/行动点/饱食/水分/经验/时间/地点 各设一个 TMP 文本（全静态搭建）。
// 字段复用：魔力→行动点、精力→饱食、金币→水分（场景已接线，沿用字段名）。
public sealed class HUD面板 : 面板基类
{
    ...
    [SerializeField] private TMP_Text 金币;      // 显示 水分度
    ...
    void Awake()
    {
        ...
        事件.订阅<生命变化事件>(e => 设文本(生命, 着(游戏主题.危险色值, $"生命 {e.当前}/{e.最大}")));
        事件.订阅<魔力变化事件>(e => 设文本(魔力, 着(游戏主题.魔攻色值, $"行动 {e.当前}/{e.最大}")));
        事件.订阅<生存状态变化事件>(e =>
        {
            if (e.类型 == 生存状态类型.饱食度) 设文本(精力, 着(游戏主题.成功色值, $"饱食 {e.当前}"));
            else if (e.类型 == 生存状态类型.水分度) 设文本(金币, 着(游戏主题.金色色值, $"水分 {e.当前}"));
        });
        事件.订阅<伤病变化事件>(e =>
        {
            if (e.当前 > 30 && e.变化量 > 0)
        ...
        事件.订阅<经验变化事件>(e => 设文本(经验, 着(游戏主题.高亮色值, $"Lv.{e.等级}  {e.当前经验}/{e.升级所需}exp")));
