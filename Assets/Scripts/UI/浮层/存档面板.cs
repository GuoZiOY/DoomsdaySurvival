...
public sealed class 存档面板 : MonoBehaviour
{
    ...
    private void 点读取(存档摘要 摘)
    {
        ...
        // ★ 刀65：先读档、**成功了才关面板** —— 失败时要留在面板上把原因说清楚，而不是黑屏让人猜。
        ...
        关闭();
        // ★ 刀65：有战局快照就**回到你离开的那一层那一格**（含翻过的柜子/开过的门/敌人位置）。
        // ★ 刀69：快照装不了时再看存档记的"层"——**在家存的档（层=安全屋）不建世界，直接回安全屋面板**。
        //   与"回安全屋会把大世界那层也收掉"（大世界探索服务.回安全屋收尾）是同一口径：在家 = 三层都没有活着的世界。
        //   这样读档就真的落在你离开时那一层（原来会落在大世界视图的营地门口格，还得自己走回去）。
