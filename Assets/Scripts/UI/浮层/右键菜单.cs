...
public sealed class 右键菜单 : MonoBehaviour
{
    ...
    public void 显示(物品堆叠 堆叠, RectTransform 物品框)
    {
        ...
        {
            if (物品.恢复量 > 0 || (物品.类型 == "书籍" && (物品.书籍种类 == "技能书" || 物品.书籍种类 == "配方书" || 物品.书籍种类 == "知识")))
                条目.Add(("使用", () => (目标面板 ?? 背包面板)?.菜单使用()));
            // 只有**装备**才给「装备」条目（v51 刀41）：
            //   原来判据是 `槽位 非空` —— 而**配件（改装件）也有槽位**（枪口/瞄具/插板…那是"配件槽"），
            //   于是菜单会给配件一个「装备」，点了必然失败（用户报的就是这个）。
            //   现在用 配件槽.认得出 把配件槽排除掉 → 剩下的才是真装备槽位。
            if (!string.IsNullOrEmpty(物品.槽位) && !配件槽.认得出(物品.槽位))
                条目.Add(("装备", () => (目标面板 ?? 背包面板)?.菜单装备()));
            // 配件：不给「装备」也不另加条目 —— 下面的通用「详情」里已写清「到工作台装配」
    ...
    public void 显示家具(物品堆叠 堆叠, RectTransform 框)
    {
        ...
        // **门**：安全屋唯一的出口 —— 只给一个选项「出去」（点一下 = 进废城）。
        //   不给 升级/拆除/详情：门是基础家具、位置固定不可动（见 家具网格面板.是门），
        //   菜单上多一行"拆除"反而让人以为能把门拆掉（拆了就没出口了）。
        if (定义标识 == "门")
        {
            条目.Add(("出去", () => ServiceRegistry.Get<大世界探索服务>()?.出门()));
            展示(条目, 框);
            return;
        }
