...
// 鼠标右键 = 物品操作菜单（见 右键菜单/网格面板.物品右键）；全局回退 由 侧边栏取消按钮 触发（右键回退已取消）。
// 测试功能（加物品/加容器/清空/随机穿戴/仓库）由 快速测试面板 按钮提供（F 键测试已移除）。
// F1 = 打开/关闭 装备与背包面板（保留为常用快捷键）。
...
public sealed class 玩家输入系统 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // 右键 = 物品操作菜单：由 网格面板 的物品点击组件处理（此处不再做全局回退）
    }
    ...
    // F1：打开/关闭 装备与背包面板（含 装备区 + 中区穿戴容器 + 右区仓库；需在 面板管理器 的「背包」引用位接线 装备背包面板）
    private void 打开背包面板()
    {
        if (面板 == null)
        {
