...
public sealed class 快速测试面板 : MonoBehaviour
{
    ...
    // 打开 制作面板（测试）：先 打开 安全屋面板，再 由 安全屋面板 打开 制作（制作面板 = 安全屋 子面板，由 它 管控）
    private void 打开制作测试(string 家具标识)
    {
        var 营地 = FindFirstObjectByType<安全屋面板>();
        if (营地 == null) { 日志("[测试] 场景缺少 安全屋面板。", true); return; }
        面板管理器.实例?.显示面板类型<安全屋面板>();
        if (!营地.gameObject.activeInHierarchy) { 日志("[测试] 安全屋面板 未接线到 面板管理器。", true); return; }
        营地.打开制作(家具标识);
    }
    ...
    private void 加制作材料()
    {
        string[] 常用 = { "木板", "钉子", "铁皮", "胶带", "零件", "布料", "绳索", "空瓶子", "火柴", "纱布", "碘伏", "棉签", "电池", "水", "医疗箱" };
        string[] 生食材 = { "土豆", "面粉", "生肉" };
        string[] 图纸 = { "军刀图纸", "消防斧图纸", "医疗包图纸", "抗生素图纸", "军用肾上腺素图纸" };
        string[] 药品 = { "绷带", "消毒酒精", "创伤药", "医疗包", "抗生素" };
        if (数据() == null || 档案() == null) return;
