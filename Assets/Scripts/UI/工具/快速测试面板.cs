...
public sealed class 快速测试面板 : MonoBehaviour
{
    ...
    //（容器战局临时数据在这一刻清空；直接 显示面板类型<安全屋面板>() 会绕过它，测不出清空时机）
    private void 回安全屋()
    {
        var 事件 = ServiceRegistry.Get<EventBus>();
        if (事件 == null) { 日志("[测试] 事件总线未装配。", true); return; }
        事件.发布(new 打开营地事件());
        日志("[测试] 回安全屋：这一趟结束（房间容器的战局临时数据已清空）。");
    }
    ...
    private void 测试战斗(string 敌人组)
    {
        ...
        放入任一穿戴容器("绷带", 4);       // 测 消耗 型 技能（绷带包扎 需 绷带）
        战斗服务.开始战斗(敌人组, "", "");
        日志($"[测试] 开始战斗：{敌人组}。");
    }
    ...
    private void 重进上一次房间()
    {
        ...
        var 服务 = ServiceRegistry.Get<房间探索服务>();
        if (服务 == null) { 日志("[测试] 房间探索服务未装配。", true); return; }
        if (!服务.进入房间(上次房间模板, 上次房间种子)) { 日志("[测试] 重进失败（看 Console）。", true); return; }
        日志($"[测试] 重进房间：{上次房间模板}（种子 {上次房间种子}）——柜子该还是你翻过的样子。");
    }
