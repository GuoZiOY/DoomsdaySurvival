...
// 逻辑：订阅 战斗开始事件 → 布阵（棋盘网格.布阵 + 技能/道具栏 重建）；Update 只做渲染（同步 单位 到 棋盘网格 + 技能槽状态）；
//  战斗时钟 由 世界时间管理器.驱动 推进（v51 刀17：不再挂在本面板的 Update 上——面板隐藏会冻结战斗）；
...
public sealed class 战斗沙盒面板 : 面板基类
{
    ...
    // **瞬时面板**：打完 / 逃跑后由 回到房间事件（或 探索显示事件）切回原面板，不占"上一个面板"
    public override bool 瞬时面板 => true;
    ...
    private void 显示结算(战斗结束事件 e)
    {
        ...
        // **格子层**（房间 / 区域 / 大世界）的战斗：尸体留在原地格子上，由玩家走过去搜（不再自动弹面板）——所以这里跳过。
        // v51 刀8 把"遭遇与战斗收尾"上移到 格子探索服务 之后，这个判断要覆盖三层（原来只判断房间层）。
        // 只有"不归属任何格子层"的战斗（剧情战斗 / 测试沙盒）才自动弹出尸体搜索面板。
        bool 格子层战斗 = ServiceRegistry.Get<房间探索服务>()?.本次战斗来自本层 == true
                       || ServiceRegistry.Get<区域探索服务>()?.本次战斗来自本层 == true
                       || ServiceRegistry.Get<大世界探索服务>()?.本次战斗来自本层 == true;
        if (e.胜利 && !string.IsNullOrEmpty(e.尸体容器) && 面板管理器.实例 != null && !格子层战斗)
        {
    ...
    // ★ v51 刀17：`推进战斗` 已搬到 `世界时间管理器.驱动.Update` —— 本面板只负责渲染。
    //   原来时钟挂在这里，而面板隐藏 = SetActive(false) → Update 停 → 战斗静默冻结（可无限"看牌"）。
    //   面板不再推进等于"没有面板也照样打"，这才是引擎该有的样子。
    void Update()
    {
        if (战斗 == null || !战斗.战斗中) return;
        if (棋盘 != null) 棋盘.同步单位(战斗.全部单位());
        刷新模式按钮();
        // 技能槽（固定 6 槽 轮询）：冷却 + 蓄力 + **可用性**（精力/弹药/消耗物/武器不对 → 压暗禁用 + 短原因）
        if (技能槽位 != null && 战斗?.玩家 != null)
            foreach (var 槽组件 in 技能槽位)
            {
                if (槽组件 == null || 槽组件.当前技能标识 == null) continue;
                string 标识 = 槽组件.当前技能标识;
                bool 可用 = 战斗.可释放技能(标识, out string 不可用原因);
                槽组件.刷新状态(战斗.玩家.冷却剩余(标识), 战斗.蓄力中技能 == 标识, 可用, 不可用原因);
            }
        ...
        if (Keyboard.current != null)
        {
            ...
            for (int i = 0; i < 6 && i < (技能槽位?.Length ?? 0); i++)
            {
                ...
                {
                    var 槽 = 技能槽位[i];
                    // ★ v51 刀17：快捷键**必须与按钮的 interactable 同口径** ——
                    //   否则"槽压暗变灰"和"按 1 照样进目标选择"就是两套说法（#9 要消灭的正是这种不一致）。
                    //   放不出来时按了不动，槽内已经写着原因（精力不足 / 需步枪 / 无弹药…）。
                    if (槽 != null && !string.IsNullOrEmpty(槽.当前技能标识)
                        && 战斗.可释放技能(槽.当前技能标识, out string 忽略原因)) { 点击技能槽(槽.当前技能标识); break; }
                }
            }
