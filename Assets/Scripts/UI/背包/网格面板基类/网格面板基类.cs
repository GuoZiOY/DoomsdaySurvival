...
public abstract partial class 网格面板基类 : MonoBehaviour
{
    ...
    //   见 定位() 与 探索图层.摆格()），所以地表盖住的那一格，底格一个像素也露不出来。
    //   唯一例外是**界外格**（楼梯间那一侧的空档）：地表故意不铺（看着就是"楼外"），
    //   底格是它唯一的地色 —— 那种格必须照建。探索网格面板 覆写本钩子只给界外格返回 true。
    //   收益（区域层 32×22）：底格 704 张 → 界外那几格；房间层 16×10：160 张 → 十几张。
    protected virtual bool 需要底格(int 列, int 行) => true;

    // 拖拽组件 开关（v51 刀19）：实体框要不要挂 `物品拖拽` 组件。
    //   探索层覆写 false —— 它们 `允许开始拖拽` 恒 false（`开始拖拽` 第一行就 return），
    //   所以挂上去的组件**永远不会做任何事**（OnBeginDrag 进去就返回，整条拖拽链路根本不会启动）。
    //   而区域层一次进图就是几百个实体框 → 白挂几百个 MonoBehaviour（每个都要进事件系统的登记表）。
