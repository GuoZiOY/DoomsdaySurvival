...
// 渲染：色块（统一底色/选中亮/内缩内容色）+ 名称 + 等级角标——内容层 与 物品 一致：
//       尺寸 恒 用"未旋转"形状（宽×格 - 边距×2），旋转 时 内容层 转 90°（色块 也 转，方向 一致）；
//       框根 尺寸 随 旋转 换向（宽/高 互换）。旧版 用"旋转后 尺寸 + 再转 90°"= 双重换向 = 显示 异常（已修）。
// 交互：单击选中（编辑模式）/双击详情/右键 家具 菜单；拖拽中 按 R 旋转（基类 通用，与 物品 一致）。
...
public sealed class 家具网格面板 : 网格面板基类
{
    ...
    {
        // 非 编辑 模式：家具 静态（单击 不 选中 高亮；双击 详情 查看 保留）
        if (家具宿主 != null && !家具宿主.编辑模式)
        {
            if (点击次数 >= 2) 家具宿主.家具被点击(堆叠);
            return;
        }
        请求刷新();   // 编辑 模式：选中 高亮（家具 选中色）
    ...
    private 物品框 创建家具框(物品堆叠 堆叠)
    {
        ...
        // 内容层：与 物品 一致——尺寸 恒 用"未旋转"形状，旋转 时 内容层 转 90°（旧版 用 旋转后 尺寸 + 再转 = 双重换向 异常）
    ...
    private void 更新家具框(物品框 框, 物品堆叠 堆叠)
    {
        ...
        // 内容层 旋转 同步（与 物品 一致：内容层 尺寸 恒 未旋转，只 转 90° 适配 框根 换向）
        if (框.内容层 != null)
        {
