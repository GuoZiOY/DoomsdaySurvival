...
public partial class 物品网格面板
{
    ...
    {
        // 跨面板：先找鼠标下方是否有别的 网格面板（基类子类：容器面板/装具块/仓库面板/制作输入输出网格）
        var 目标面板 = 事件下方面板(事件);
        if (目标面板 != null && 目标面板 != this)
            return 目标面板.接收跨面板转移(服务, 源, 事件, 拖拽旋转);   // 传拖拽旋转：跨面板 R 旋转 生效
        // 浮层 底座 拦截：鼠标在 浮动面板 上 → 落点 归 浮层；叠放取最上层；
        // 自己 宿主：网格 内 松手 = 同面板 移动/换位（跳过 装备槽 分支——穿透 修复）
        var 自己宿主 = 浮动宿主;
        var 最上层浮动 = 最上层浮动宿主(事件);
        bool 在自己宿主网格内 = false;
        if (最上层浮动 != null)
        {
            if (最上层浮动 != 自己宿主) return false;   // 其他 浮动面板 上 → 拦截
            if (!屏幕命中(事件.position)) return false;  // 底座 空白 区 → 拦截
            在自己宿主网格内 = true;
        }
        // 拖到装备槽位（装备区）→ 穿戴：槽位兼容才可穿（实例级换装到"命中槽位"，旧件回背包/回滚已处理）
        if (!在自己宿主网格内 && 装备区.实例 != null && 装备区.实例.命中槽位(事件.position, out var 槽位名) && 源 != null)
        {
