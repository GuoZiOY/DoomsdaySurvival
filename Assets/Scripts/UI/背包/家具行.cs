...
public sealed class 家具行 : MonoBehaviour
{
    ...
    public void 绑定(string 名称文本, string 描述文本, string 信息文本, bool 可建造, bool 可升级, bool 材料足够,
        UnityEngine.Events.UnityAction 建造回调, UnityEngine.Events.UnityAction 升级回调)
    {
        ...
        if (可建造)
        {
            if (文本 != null) 文本.text = "建造";
            if (按钮 != null)
            {
                按钮.onClick.RemoveAllListeners();
                按钮.onClick.AddListener(材料足够 ? 建造回调 : 材料不足反馈);
            }
        }
        else
        {
            if (文本 != null) 文本.text = "升级";
            if (按钮 != null)
            {
                按钮.onClick.RemoveAllListeners();
                按钮.onClick.AddListener(材料足够 ? 升级回调 : 材料不足反馈);
            }
        }
    }
