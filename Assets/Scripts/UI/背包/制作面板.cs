...
public sealed class 制作面板 : 浮动面板基类
{
    ...
    protected override bool 可否关闭()
    {
        ...
        if (!输入全回 || !输出全回)
        {
            ServiceRegistry.Get<EventBus>()?.发布(new 日志事件(日志类型.反馈坏, "背包/仓库已满，部分材料或成品未能放回——请先腾出空间再关闭制作面板。"));
            音效管理器.实例?.播放失败();
    ...
    private void 选中配方(string 标识)
    {
        ...
        if (!返还会话物品(输入会话))
        {
            ServiceRegistry.Get<EventBus>()?.发布(new 日志事件(日志类型.反馈坏, "背包/仓库已满，无法取回输入区材料——请先腾出空间。"));
            音效管理器.实例?.播放失败();
            if (输入网格 != null) 输入网格.立即刷新();
            return;
        }
    ...
    private void 刷新详情()
    {
        ...
        float 消耗 = 制作.消耗预期(配方) * 制作数量;
        详情文本.text = $"【{物品名(配方.产物)} ×{配方.产物数量 * 制作数量}】\n\n{配方.描述}\n\n材料：{制作.材料文本(配方, 制作数量)}\n耗时：{耗时}　精力：{配方.消耗精力 * 制作数量}{图纸}\n饱食/水分：-{消耗:F1}";
