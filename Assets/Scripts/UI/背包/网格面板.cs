...
// 数据源：null = 主背包（档案.背包服务）；非空 = 外部视图（容器/仓库/穿戴容器，由 容器面板/仓库面板/装具区 注入）。
public sealed partial class 网格面板 : 面板基类
{
    ...
    // 网格数据源：由 容器面板/仓库面板/装具区 注入（容器/仓库/穿戴容器 视图）。
    ...
    // 外部请求刷新入口（装具区/仓库面板/容器面板 注入数据源/尺寸后调用）：内部自动判定 全量重建 或 增量刷新
    public void 请求刷新() => 待刷新网格 = true;
    // 立即刷新（装具区 装备/卸下 后强制显隐生效用）：直接执行（低频结构变化，不走脏标记）
    ...
    // 外部视图显示配置（容器面板/仓库面板/装具区 注入）：格尺寸 固定常量（网格面板.格尺寸）；
