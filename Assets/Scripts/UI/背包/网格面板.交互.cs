...
public sealed partial class 网格面板
{
    ...
    private void 物品被点击(物品堆叠 堆叠, int 点击次数)
    {
        ...
        if (家具宿主 != null && 家具宿主.摆放中) return;   // 摆放 模式：家具 点击 忽略（左键 用于 落格 建造）
        右键菜单.实例?.隐藏();   // 左键点击：先关闭右键菜单
        选中 = 堆叠;
        if (家具宿主 != null)
        {
            请求刷新();   // 选中 高亮（家具 选中色）
            if (点击次数 >= 2) 家具宿主.家具被点击(堆叠);   // 双击：详情/提示（安全屋面板 处理）
            return;
        }
        if (点击次数 >= 2)
        {
    ...
    {
        if (堆叠 == null) return;
        if (家具宿主 != null && 家具宿主.摆放中) return;   // 摆放 模式：右键 忽略
        选中 = 堆叠;
        if (家具宿主 != null)
        {
            // 家具：右键菜单 家具 模式（升级/拆除/详情；旋转 = R 键，移动 = 拖拽）
            if (右键菜单.实例 != null)
            {
                右键菜单.实例.目标面板 = this;
                右键菜单.实例.显示家具(堆叠, 物品框);
