...
public sealed partial class 网格面板
{
    ...
    private static readonly Color 物品描边色 = new Color(0f, 0f, 0f, 0.6f);      // 物品框 黑描边
    private static readonly Vector2 物品描边距离 = new Vector2(3f, -3f);
    private float[] 块偏移;
    ...
    // 装具块（所属槽位 非空，中区 三块）不需要 底盘——纯代码判定，无需手动配置。
    private void 创建底盘()
    {
    ...
    // ② 高光层：品质底 与 内容 之间（白色半透明，悬停时显示；覆盖到 网格线 上）
    private static GameObject 创建高光层(Transform 父)
    {
    ...
    // 数量 总是创建（≤1 隐藏、>1 显示）——否则 合并/拆回 数量>1 无法补显（拆分 5→4+1 再合并 回 5 的 bug）
    private void 创建文本角标(Transform 父, 物品堆叠 堆叠, 物品数据 物品, 物品框 框)
    {
        ...
        UI工具.设锚(数.rectTransform, new Vector2(1, 0), new Vector2(1, 0), new Vector2(-5f, -2f), new Vector2(70f, 34f));   // 右下顶角
        数.gameObject.SetActive(堆叠.数量 > 1);   // 初始：>1 显示（≤1 隐藏，更新 时 自动 切换）
        框.数量 = 数;
        框.上次数量 = 堆叠.数量;
    }

    // ④ 点击（非按钮） + 拖拽（挂在物品框上）
    private void 挂接交互(物品框 框, 物品堆叠 堆叠, RectTransform 内容矩形)
    {
    ...
    // ===== UI 创建（统一走 UI工具，见 UI/框架/UI工具.cs） =====

    // 分隔线宽：物品边界（亮且非形状线）或 口袋 加粗（有物品）→ 物品边界线宽；否则 普通线宽
