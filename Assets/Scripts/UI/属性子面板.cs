...
// 自刷新：订阅 属性/生命/魔力变化事件；父面板只负责 Tab 切换，不参与本页内容。
public sealed class 属性子面板 : MonoBehaviour
{
    // 四维行（索引 = 体力/力量/智力/敏捷）；行 = 值文本 + 加点按钮
    [SerializeField] private 属性行引用[] 四维;
    // 派生能力值文本（索引 = 生命/魔力/物伤/魔伤/暴击/闪避）
    [SerializeField] private TMP_Text[] 能力值;

    private static readonly 属性类型[] 四维类型 = { 属性类型.体力, 属性类型.力量, 属性类型.智力, 属性类型.敏捷 };
    ...
    public void 刷新()
    {
        ...
        // 四维（值 + 加点按钮显隐）
        var 四维值 = new[] { 玩家.体力, 玩家.力量, 玩家.智力, 玩家.敏捷 };
        ...
        // 派生能力（生命/魔力只显示上限——属性面板看成长上限，不看随时变化的当前值）
        var 能力文本 = new[] {
            $"{玩家.最大生命}", $"{玩家.最大魔力}",
            $"{玩家.物理伤害}", $"{玩家.魔法伤害}",
