...
public sealed class 房间网格面板 : 探索网格面板
{
    ...
    protected override void 更新实体框(物品框 框, 物品堆叠 堆叠)
    {
        ...
        // 可见性按"档"：玩家永远画；敌人只在 亮（核心）里画；墙/门 只要不是全黑就画（白天记忆 / 夜晚弱视野都看得见轮廓）；
        ...
        {
            if (实体.是玩家) 显示 = true;
            else if (实体.类型 == 网格实体类型.敌人) 显示 = 亮;
            else if (实体.类型 == 网格实体类型.墙) 显示 = 档 != 视野档.全黑;
            else if (实体.类型 == 网格实体类型.门) 显示 = 档 != 视野档.全黑;   // 门和墙一样：黑记忆/夜晚弱视野里都看得见轮廓
            else if (实体.类型 == 网格实体类型.楼梯) 显示 = 档 != 视野档.全黑;   // 楼梯同理：走到跟前得看得见"楼梯在哪"
            else 显示 = 亮 || 档 == 视野档.暗记忆;
        }
