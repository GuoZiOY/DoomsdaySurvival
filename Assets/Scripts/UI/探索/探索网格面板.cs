...
public abstract class 探索网格面板 : 网格面板基类
{
    ...
    // ================= 实体框每帧更新（v51 刀21b 上提） =================
    // 区域层与大世界层这两个 `更新实体框` 原来是**逐行相同的两份**（只有注释里的层名不同；
    // 复扫工具 Tools\重复代码扫描.ps1 报 100% / 16 行），所以收进基类一份。
    // 房间层不一样（它要换门图、写名称、按"已翻过"灰化）→ 由 房间网格面板 自己覆写。
    protected override void 更新实体框(物品框 框, 物品堆叠 堆叠)
    {
        if (框?.根 == null || 堆叠 == null) return;
        var 实体 = 找实体(堆叠.标识);
        var (宽, 高) = 格数(堆叠);
        // 玩家令牌：位置由 图层 平滑接管时，这里不要把它拽回格子（否则会和插值打架）
        ...
        //   ⚠ 修正记录（v52）：区域层与大世界层原来都写的是 `else 显示 = 亮 || 档 == 暗记忆;`，
        //     而**敌人落进了这个 else** → 站在街上就能看见全街区/全城的敌人（房间层是对的）。
        //     迷雾改永久保留之后这个 bug 会致命（街区退化成纯绕路题），所以一并收进共用规则。
        //   地形类（墙/区域/建筑/临时建筑）：不是全黑就留轮廓 —— "那片街区我去过"一眼看得出来。
        //   多格实体（楼 / 区域 / 营地）：档取占地里最亮的那一格（看见一个角，整块就画出来，剩下由迷雾层盖黑）。
        var 档 = 整体档(实体);
    ...
    // 多格实体（楼 / 区域 / 营地）的"整体档"：取占地掩码里**最亮**的那一格 ——
    // 只看见一个角，整块就该画出来（没看见的部分由 迷雾层 逐格盖黑，不必整块一起藏）。
    // 单格实体退化成 档(列,行)，与房间层那些单格实体的判定等价。
    protected 视野档 整体档(网格实体 实体)
    {
        var 服 = 探索服务;
        ...
        for (int r = 实体.行; r < 实体.行 + 实体.高; r++)
            ...
            {
                if (!实体.覆盖(c, r)) continue;      // 非矩形的空缺（凹口）不算
                var 档 = 服.档(c, r);
                if (档 > 最好) 最好 = 档;
