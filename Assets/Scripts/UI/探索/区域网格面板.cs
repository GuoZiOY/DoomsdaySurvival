...
public sealed class 区域网格面板 : 探索网格面板
{
    ...
    protected override void 更新实体框(物品框 框, 物品堆叠 堆叠)
    {
        ...
        // 可见性按"档"：规则**唯一一份**在 网格可见性.显示()（与房间层/大世界层共用）。
        //   ⚠ 修正记录（v52）：这里原来写的是 `else 显示 = 亮 || 档 == 暗记忆;`，而**敌人落进了这个 else**
        //     → 敌人会被画在"记忆层"里 = 站在街上一眼看见全街区的敌人（房间层是对的）。
        //     迷雾改永久保留之后这个 bug 会致命（街区退化成纯绕路题），所以收进共用规则并钉了断言。
