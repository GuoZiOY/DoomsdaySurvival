...
public sealed class 区域网格面板 : 探索网格面板
{
    ...
    protected override void 更新实体框(物品框 框, 物品堆叠 堆叠)
    {
        ...
        // 可见性按"档"（与房间层同一套规矩）：楼 / 墙 = 只要不是全黑就画（记忆里留轮廓，夜里也看得见楼影）；
        // 障碍 = 只在亮 / 白天记忆里画（黑暗里看不见废车，免得一头撞上去）；玩家永远画。
        ...
        if (实体 != null && !实体.是玩家)
        {
            if (实体.类型 == 网格实体类型.建筑) 显示 = 档 != 视野档.全黑;
            else if (实体.类型 == 网格实体类型.墙) 显示 = 档 != 视野档.全黑;
            else 显示 = 亮 || 档 == 视野档.暗记忆;   // 障碍（和房间层的容器/尸体同一条）
        }
    ...
    private void 打开菜单(网格实体 实体, RectTransform 框)
    {
        ...
        if (实体.类型 != 网格实体类型.建筑 && 实体.类型 != 网格实体类型.障碍) return;
        右键菜单.实例.目标面板 = this;
