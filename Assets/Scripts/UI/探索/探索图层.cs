...
public abstract class 探索图层 : MonoBehaviour, 探索图层接口
{
    ...
    // 路径高亮要跳过队首几格（v51 刀22，防"高亮拖在角色后面"）：0 = 令牌停在格心（正常画全）；
    // 1 = 令牌已开始走向下一格 → 刚离开的那一格立刻不再点亮
    private int 路径起点偏移;
    ...
    private void 重建()
    {
        ...
        上次迷雾版本 = int.MinValue; 上次迷雾压暗 = -1f; 上次路径签名 = int.MinValue; 上次实体签名 = int.MinValue;
    ...
    private void 切世界(格子探索服务 服)
    {
        ...
        段计时 = 段总时 = 0f;
        路径起点偏移 = 0;   // 换了世界：偏移作废（新世界的 路[0] 就是令牌脚下那一格）
        视觉位置 = Vector2.zero;
        悬停列 = 悬停行 = -1;
        落点列 = 落点行 = -1;
        落点剩余 = 0f;
    ...
    private void 刷路径(bool 强制 = false)
    {
        var 服 = 服务;
        var 路 = 服?.当前路径;
        // ★ v51 刀22：**可以跳过队首那几格**（用户反馈的"高亮拖在角色后面"）。
        //   逻辑格要等令牌**踩到格心**才推进（推进一步），但令牌是连续走过去的 ——
        //   0.3 秒的移动里"脚下那一格"其实早就离开了，若还亮着，看起来就是一条拖尾。
        //   所以 推进移动 在"开段"时把 路径起点偏移 置 1，这一段的高亮立刻从 路[1] 开始画。
        //   偏移进签名：0→1 时签名变了 → 这一次收尾会真的重画（不靠强制）。
        int 首 = (路 != null && 路.Count > 1) ? Mathf.Clamp(路径起点偏移, 0, 路.Count - 1) : 0;
        int 签 = 首 * 7919 + (路 == null ? 0 : 路.Count);
        if (路 != null)
        ...
        if (路 != null && 路.Count > 首)
        {
            for (int i = 路.Count - 1; i >= 首 && 用 < 路径池.Count; i--, 用++)
                摆格(路径池[用], 路[i].列, 路[i].行);
            int 点用 = 0;
            for (int i = 路.Count - 2; i >= 首 && 点用 < 点线池.Count; i--, 点用++)
            {
                var 图 = 点线池[点用];
                if (!图.enabled) 图.enabled = true;
    ...
    private void 推进移动()
    {
        ...
        float 格 = 网格面板基类.格尺寸;
        if (服.遭遇中) { 段有效 = false; 路径起点偏移 = 0; 视觉位置 = 格位(服.玩家列, 服.玩家行, 格); return; }   // 遭遇：停在这一格
        if (!段有效) 视觉位置 = 格位(服.玩家列, 服.玩家行, 格);   // 没在走：令牌贴在逻辑格中心（含刚走完/被拽回）

        var 路 = 服.当前路径;
        if (路 == null || 路.Count <= 1) { 段有效 = false; 路径起点偏移 = 0; 视觉位置 = 格位(服.玩家列, 服.玩家行, 格); return; }
        var 目标 = 格位(路[1].列, 路[1].行, 格);
        if (!段有效 || 段目标 != 目标) 开段(目标);   // 新一步 / 点了别处 → 从当前位置重新开一段

        // ★ v51 刀22（用户反馈"路径高亮消失得比角色慢，看着像拖尾"）：
        //   令牌这一段已经在往下一格走了 → **刚离开那一格的高亮立刻收掉**（路径从 路[1] 开始画）。
        //   不改逻辑：逻辑格照样等踩到格心才 推进一步（视野/迷雾的推进节奏一字未动）—— 只动"哪个格子亮着"。
        //   代价：每开一段多发一次 刷路径（从尾分配，只关掉一张图，几乎免费）。
        if (路径起点偏移 != 1) { 路径起点偏移 = 1; 刷路径(); }
        ...
        // 逻辑格推进后 路[0] 就是令牌脚下那一格 → 偏移归零（刚才收掉的正是"上一格"，现在该亮的是这一格）
        路径起点偏移 = 0;
    ...
    private int 上次实体签名 = int.MinValue;  // 实体位置签名的滚动哈希

    // ★ 刀61：**实体会动**了（大世界敌人会巡街）—— 这是本项目第一次出现"除了玩家之外会挪格的实体"。
    //
    // 为什么不能只靠"显示事件"那条路：`大世界面板.刷新网格()` 第一行就是
    //   `if (!gameObject.activeInHierarchy) return;` —— 面板那一刻不激活（战斗中 / 切到别的面板），
    //   这次刷新就被丢掉；而**服务侧的敌人已经挪过去了**。丢掉一次之后没有任何补刷，
    //   敌人令牌就**永远停在旧格**上，还不报任何错（表现就是"敌人根本没动过"）。
    //
    // 做法与本文件已有的两个去重键**完全同一个套路**（刷迷雾 按 视野版本、刷路径 按 路径签名）：
    //   这里算一个"实体位置签名"，变了才请面板重摆 —— 与事件路径互为保险，任一通了就画对。
    private void 刷实体()
    {
        var 服 = 服务;
        var 世界 = 服?.当前世界;
        if (世界 == null || 面板 == null) return;
        unchecked
        {
            int 签 = 世界.实体.Count * 486187739;
