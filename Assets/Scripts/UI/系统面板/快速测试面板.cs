...
public sealed class 快速测试面板 : MonoBehaviour
{
    ...
    // 通知 UI 刷新：背包变化事件（装备面板/穿戴容器区/网格）+ 属性变化事件（属性/穿戴容器区/网格）
    ...
    private void 打开背包面板()
    {
        ...
        if (面板.当前显示面板 is 装备背包面板) { 面板.返回上一面板(); return; }
        面板.显示面板类型<装备背包面板>();
        if (面板.当前显示面板 is not 装备背包面板) 日志("[测试] 装备与背包面板未接线到 面板管理器。", true);
    }

    // 加 1 个随机物品：扫描 弹挂/腰封/背包 三个穿戴容器，任一有空位即放入
    private void 加随机物品()
    {
        ...
        int 实际 = 放入任一穿戴容器(标识, 1);
        事件().发布(new 背包变化事件(标识, 实际, 变化原因.获得));
        string 名称 = 数据().物品.TryGetValue(标识, out var 物) ? 物.名称 : 标识;
        if (实际 <= 0) 日志($"[测试] 弹挂/腰封/背包 均无空位，{名称} 放不下了！", true);
        else 日志($"[测试] 获得 {名称} ×{实际}");
    }

    // 扫描 弹挂/腰封/背包 三个穿戴容器：逐个尝试放入，返回实际放入数量
    ...
    // 一键清空 主背包网格 + 弹挂/腰封/背包 三个穿戴容器（装备槽本身保留）
    private void 清空背包()
    {
        var 玩家 = 档案();
        if (玩家 == null) return;
        int 主背包数 = 玩家.背包.Count;
        int 容器数 = 0;
        foreach (var 槽 in new[] { "弹挂", "腰封", "背包" })
        {
            var 记录 = 玩家.装备.Find(e => e.槽位 == 槽);
        ...
        玩家.背包.Clear();
        int 总数 = 主背包数 + 容器数;
        if (总数 <= 0) { 日志("[测试] 背包与穿戴容器均为空，无需清空。"); return; }
        事件()?.发布(new 背包变化事件("", 0, 变化原因.失去));   // 触发 网格/穿戴容器区 刷新
        日志($"[测试] 已清空主背包（{主背包数} 件）与穿戴容器（{容器数} 件）。");
    }
