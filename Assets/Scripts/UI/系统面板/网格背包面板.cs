...
public sealed class 网格背包面板 : 面板基类
{
    ...
    void Update()
    {
        // 脏标记合并刷新：事件（背包/属性变化）只标记，Update 帧末合并一次重建——装备等操作同帧多次事件不重复重建（优化卡顿）
        if (待刷新网格) { 待刷新网格 = false; 刷新网格(); }
        if (待刷新信息) { 待刷新信息 = false; 刷新信息(); }
    ...
    private bool 待刷新网格, 待刷新信息;   // 脏标记：事件 → 标记，Update 合并刷新（避免同帧多次全量重建）

    // 事件响应：只标记脏，Update 统一刷新（优化：装备/转移同帧多个事件不重复重建）
    private void 背包变化响应(背包变化事件 _) => 待刷新网格 = true;
    private void 属性变化响应(属性变化事件 _) => 待刷新信息 = true;
    ...
    // 拆分面板确认 → 执行拆分（安全放置：本网格空位优先；容器拆分时容器满 → 兜底放主背包；都满 → 不允许）
    public void 菜单拆分(int 数量)
    {
        ...
        bool 容器视图 = 服务 != 主;
        var 事件 = ServiceRegistry.Get<EventBus>();
        // ① 本网格拆分（主背包 或 容器内）：拆出的新堆叠 自动找本网格空格
        var 新 = 服务.拆分(选中, 数量);
        if (新 != null)
        {
            音效管理器.实例?.播放成功();
        ...
        // ② 容器无空位 → 兜底放主背包（源堆叠仍在容器内减量，新堆叠进主背包）
        if (容器视图)
        {
            var 兜底 = 主.拆分(选中, 数量);
            if (兜底 != null)
            {
                ...
                选中 = null;
                刷新网格();
                事件?.发布(new 日志事件(日志类型.反馈, $"[背包] 容器已满，拆分出的 {兜底.标识}×{数量} 放入主背包。"));
                事件?.发布(new 背包变化事件(兜底.标识, 数量, 变化原因.获得));   // 主背包刷新
                return;
            }
        }
        // ③ 容器与背包均无空位 → 不允许拆分（安全保护：不丢物品）
        音效管理器.实例?.播放失败();
        事件?.发布(new 日志事件(日志类型.反馈坏, "[背包] 拆分失败：容器与背包均已满，无空位放置拆出物品。"));
    }
    ...
    private void 结束拖拽(PointerEventData 事件)
    {
        ...
        if (装备面板.实例 != null && 装备面板.实例.命中槽位(事件.position, out var 槽位名) && 源 != null)
        {
            ...
            {
                if (面板操作.换装堆叠(档案, 源, 槽位名, 服务)) { 刷新网格(); }   // 指定目标槽 + 源服务（穿戴容器/主背包）
                else 音效管理器.实例?.播放失败();
            }
            else 音效管理器.实例?.播放失败();
            return;
