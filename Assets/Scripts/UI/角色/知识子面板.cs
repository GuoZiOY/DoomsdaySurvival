...
//   · 左 = **滚动视窗**里的知识列表：一行只有「名称 · 等级（罗马数字）· 选中高亮 · 未掌握置灰」四样（见 `知识行`）。
//     ⚠ **未掌握的行不藏起来**（本批用户口径）：13 本知识**全部**显示（已掌握排前面，排序不动），
//       未掌握的靠 `知识行.未掌握置灰` 那个**预置节点的显隐**表达 —— 代码只 `SetActive`，不写颜色/透明度。
//   · 右 = 详情区：**只有一个 `TMP_Text`**，所有细节按行拼进这一个文本（`《名称》` / 家族 · 领域 / 等级 /
//     当前经验 / 逐级表）。
...
//     （选中高亮 = 一个预置节点亮了而已，不写颜色；未掌握置灰 = `知识行` 里另一个预置节点的显隐，同样不写颜色）。
//     ⚠ **唯一的有意例外（本批用户定稿）**：逐级表要按"到手 / 没到手"用**富文本颜色**标注 → 本页为此多出
//       2 个 `[SerializeField] Color`（`已获得色` = 白 / `未获得色` = 灰）
//       + `ColorUtility.ToHtmlStringRGB`（只把 `Color` 转成 `#RRGGBB`）。
//       为什么这仍不算破边界：**色值不在代码里**（两个字段在 Inspector 上可调，代码只做 Color → 十六进制的转换），
//       只是表达方式从"场景里摆一个预置节点"换成了"组件上的序列化字段"。除这两处外，本页与 `知识行` 的
//       外观 API 仍是 0 处（不写字号 / 不算尺寸 / 不碰 `CanvasGroup`；`SetActive` 与文本内容才是其余状态手段）。
//       ⚠ 前提：承载详情的那个 `TMP_Text` 必须开着 **richText**（TMP 默认就是开的；本项目所有序列化过的
//       TMP 组件在场景/预制体里都是 `m_isRichText: 1`）—— 关掉的话这些 `<color=…>` 会原样当可见文字画出来。
//     行高由场景里 `知识行模板` **自己的 RectTransform**
...
//   · 经验**显示口径**（本批用户定稿）：这一行只回答"我在这一级里攒了多少 / 这一级要攒多少" ——
//     `当前经验: {级内经验} / {本级所需}`（见 `经验文本`）；页面上**两个字都不提"累计"**。
//     底层语义**一个字都不动** —— `知识进度.经验` 仍是**累计**值、`成长管理器.加知识经验` 的升级判定仍是
//     `进度.经验 >= 门槛[级]`：本页只是把那个累计值**换算成级内分数**给人看。
public sealed class 知识子面板 : MonoBehaviour
{
    ...
    // —— 逐级表两态色（用户定稿：**已解锁 = 白、未解锁 = 灰，不要第三种颜色**）——
    // 为什么是**字段**而不是常量：颜色要能在 Inspector 上调；代码只管把 `Color` 转成 `#RRGGBB` 塞进富文本，
    //   所以"想换配色"不需要改代码（写死常量的话，用户每换一次色都得让程序改一遍源文件）。
    // 为什么只有两态（上一版的"下一级"那一档单独染色已按用户口径取消）：逐级表要回答的只有
    //   "这一级我拿到了没有"；再给"下一级"一种颜色，读者会以为那是**第三种状态**（比如"正在升级中"），
    //   而不是"再攒一点就到"。
    // ⚠ 灰那一个的值比 hex 的精确商略高一丁点：`ColorUtility.ToHtmlStringRGB` 内部是 `Color → Color32`，
    //   那一步是**截断**而不是四舍五入 —— 写精确商（`138/255 ≈ 0.5411765`）会因浮点误差落到 137，
    //   输出 `898378` 而不是 `8A8378`；这里每个分量都离截断边界 0.03/255 以上，算出来稳定等于下面的 hex。
    [SerializeField] private Color 已获得色 = new Color(1f, 1f, 1f);              // → #FFFFFF 白（已解锁）
    [SerializeField] private Color 未获得色 = new Color(0.542f, 0.514f, 0.471f);   // → #8A8378 次要灰（未解锁）
    ...
    // 绑定一行 —— **行里只有四样**（用户定稿）：名称（品质着色）· 等级（罗马数字）· 选中高亮 · 未掌握置灰。
    //   当前经验与逐级表都归右详情区（行是索引，不是台账）。
    //   `未掌握` = 级 ≤ 0：页面只把这个**事实**告诉行，怎么"灰"是场景里那个预置节点的事（代码不写颜色/透明度）。
    private void 绑行(知识行视图 行, 玩家档案 玩家, string 标识, UnityEngine.Events.UnityAction 点击)
    {
        ...
        // ⚠ **未掌握的行也照常显示**（本批用户口径）：不 SetActive(false) —— 13 本全在列表里，只是灰着。
        // 版式（用户定稿）：整行读作 `《近战精通》· II` —— 书名号与《详情》标题同源（`《` + 标识 + `》`，
        //   且同样包在品质着色里面）；等级那一栏是 **`· ` + 罗马数字**。
        //   为什么那个 `·` 在**代码里**而不是场景里：行模板（`知识行.prefab`）只有 `名称` / `等级` 两个文本节点，
        //   没有第三个"分隔符"节点 —— 分隔符不放代码里就根本不会出现；而它与两侧文本同属"这一行写什么"，
        //   归拼串管比在预制体里多摆一个节点更好改（改一次两处都变）。
        //   为什么挂在等级那一栏的**前面**：等级是这一行的次要栏，分隔号跟着它走 —— 将来把等级右对齐也不会脱节。
    ...
    // 详情拼法（用户定稿的顺序）：
    //   [块1] `《标识》`（**书名号包在品质着色里**——`名称着色` 只是把整段染一遍，与书名号不冲突）
    //         / `家族 · 领域` / `等级 3/ 10`（阿拉伯；前面那两个字面 `等级 ` 由 `刷详情` 补）/ `当前经验 a / b`（满级 = `已满级`）
    //   [块2] 逐级表（每级一行 `{级}级：{奖励}`，两态用**富文本颜色**：已解锁 = 白、未解锁 = 灰）
    // 为什么用**空行分块**：上面几行是"一眼扫的当前状态"，逐级表是"逐条看的清单"，中间空一行才分得开。
    // ⚠ 本批按用户口径**再删两块**（整块删掉，不留空块、不留死代码）：
    //   · `物品数据.描述`（如"枪油味还没散尽的训练手册…"那一长句）—— 那是给**书籍物品面板**"读"的文案；
    //     知识页这一栏是"看进度"的，十本书十段散文挂上去，反而把真正要看的等级清单挤下去。
    //   · 单独一行的"本条知识合计加成"（如孤零零一句 `攻击 +2`）—— 每一级的加成**逐级表里已经逐条写了**
    //     （`1级：暴击 +2%`），再单独汇总一遍就是同一件事写两遍；连带 `合计加成()` 这个**只服务于它**的
    //     取值方法一并删（没有第二个调用者，留着就是死代码）。
    // 为什么没选中就直接清空文本：详情只有一个节点，没有"半块要收起"的东西；空文本 = 什么都没显示，
    //   而 SetActive 反而不安全（用户可能把详情文本挂在页根附近，一关就把整页关掉）。
    // ⚠ 上一版**已删掉"持有 M 本"那一行**（用户定稿）：那是"背包里还躺着几本"的台账数，与"这门知识我练到哪"
    //   无关；摆在这一串里的第三行，还会被读成"升级要消耗 N 本"。`持有文本()` 也已删净。
    private void 刷详情(玩家档案 玩家, DataService 库)
    {
        ...
        int 累计经验 = 玩家.查知识(书.标识)?.经验 ?? 0;  // **累计**口径（`知识进度.经验`）；本页只把它换算成"级内分数"

        // [块1] 标题与列表行同源 —— `物品数据` 没有 `名称` 字段，标识即显示名。
        //   标题用**书名号**（用户定稿）：`《近战精通》`。书名号包在**品质着色里面**（不是"染了色就没有书名号"）：
        //   `名称着色` 只给整段加一层 `<color>`，符号与文字同色 —— "染成什么色"仍只由 `品质档` 决定。
    ...
    // 等级：`3/ 10`（阿拉伯数字；详情头部前面还有字面的 `等级 ` 二字 → 整行读作 `等级 3/ 10`）。
    //   为什么详情里**不用罗马数字**（用户定稿）：罗马只适合"短、扫一眼"的位置（列表行那一栏），
    //   详情这一行要和右边的 `10` 直接比大小，`III/ 10` 得先在脑子里翻译一遍才知道离满级还差几级。
    //   未掌握（级 = 0）→ `0/ 10`：阿拉伯口径下 0 本身就是"还没入门"，不必再换一个破折号（换了反而像"没数据"）。
    //   罗马数字仍保留给**列表行**（`知识行.等级`），那边只有一列窄位、只表达"第几级"。
    private static string 等级文本(物品数据 书, int 级) => $"{级}/ {级数(书)}";

    // 经验：`当前经验: {级内经验} / {本级所需}`（本批用户定稿）。分子 = **这一级里**攒了多少；分母 = 这一级要攒多少。
    //   · 级内经验 = 累计经验 − 基准；本级所需 = 门槛[当前级] − 基准 —— **两个数都由真实门槛算出来，没有写死的值**。
    //   · **基准** = 这一级之前"已经用掉"的那部分累计门槛：
    //       当前级 ≥ 2 → `知识门槛[当前级 - 1]`（升到当前级那一次用掉的那一档）；
    //       当前级 ≤ 1（未掌握 / 刚解锁）→ **0**。
    //     为什么 1 级也按 0 起算（用户口径里点明的那件事）：`门槛[0]` 那档**在代码路径上取不到** ——
    //       `成长管理器.加知识经验` 首次读满就**直接下发 1 级**（`new 知识进度 { 等级 = 1, 经验 = 0 }`），
    //       玩家从没为 1 级攒过 `门槛[0]`；而升级判定是 `进度.经验 >= 门槛[进度.等级]`（**累计**口径）→
    //       1 级要攒够的其实是 `门槛[1]`。所以 1 级的分母只能是 `门槛[1] - 0`：若照"前后两档相减"硬减 `门槛[0]`，
    //       1 级会从 `0 / 100` 起步、却要攒到累计 200 才升级 → 界面上出现 `150 / 100` 却纹丝不动，是句假话。
    //   · 未掌握（级 = 0）→ `0 / 门槛[0]`（用户口径明确要 `门槛[0]` 当这一档的分母）。
    //   · 满级（级 ≥ 级数）→ `已满级`（门槛表里没有"下一档"，再报分数反而是假的）。
    //   ⚠ 这里**只读** `知识进度.经验`，不写回、也不参与任何判定 —— 存储口径（累计）与 `成长管理器` 一个字都没动，
    //     本批改的只是"给人看的那句话"。
    private static string 经验文本(物品数据 书, int 级, int 累计经验)
    {
        int 上限 = 级数(书);
        if (上限 > 0 && 级 >= 上限) return "已满级";           // 满级：门槛表里没有"下一档"，写数字反而是假的
        int 基准 = 级 >= 2 ? 门槛取(书, 级 - 1) : 0;           // 见上：1 级与未掌握都以 0 为基准
        int 级内 = Math.Max(0, 累计经验 - 基准);               // `Max` 只是防坏档（经验居然小于基准）时显示负数
        int 所需 = Math.Max(0, 门槛取(书, 级) - 基准);
        return $"当前经验: {级内} / {所需}";
    }

    // 本条知识合计加成**已整块删除**（用户口径："不需要额外写出单独的『攻击 +2』"）——
    //   逐级表里每一级已经逐条写了加成（`1级：暴击 +2%` / `5级：命中 +3%`），汇总块是同一件事写两遍。
    //   删掉的是整个 `合计加成(书, 当前级)` 方法体（原来做 `加成类型` 分组累加）——它是**只服务于那个汇总块**的取值，
    //   没有第二个调用者，留下就是死代码。`按百分点` 仍在用（`解锁文本` 里逐级行的 `暴击 +2%` 靠它定单位）。


    // ================= 逐级表（拼进详情文本） =================

    // 逐级表：1..满级 每级一行，**两态用富文本颜色**（本批用户定稿：已解锁 = 白、未解锁 = 灰，
    //   取代上一版的 `√ / → / ·` 前缀与"下一级"单独染色）——
    //   · 已解锁（级 ≤ 当前级）→ `已获得色`（白）；· 未解锁（级 > 当前级）→ `未获得色`（灰）。**只有这两态**。
    //   为什么改用颜色而不是符号：符号要占一列，把"1级"整列往右推，而它并不携带读者真正需要的信息；
    //   颜色是**零宽度**的标注 —— 一列扫下去同样看得出进度到哪，行首省下的宽度全留给奖励本身。
    //   为什么不要"下一级"那第三色：逐级表只回答"到手没有"，第三色会被读成"另一种状态"。
    //   ⚠ 行内容**只有** `{级}级：{奖励}`（例：`1级：暴击 +2%` / `2级：解锁[割裂]` / `6级：解锁[横扫]、[收割]`）：
    //     本批把**门槛从逐级行里彻底去掉**（用户定稿）—— 门槛只在详情头部的"当前经验"那一行出现**一次**；
    //     逐级表回答的是"这一级给什么"，不再回答"要攒多少"（两处都写 = 同一件事写两遍，改口径时必有一边漏）。
    //     于是 `门槛取(书, i)` 在逐级行里的那一段没了，上一版"满级那一行不写门槛"的例外也**自然作废**（现在压根不写）。
    //   · 某一级没有任何奖励（数据没配全）→ 只写 `{级}级`，**不带那个全角冒号**：
    //     写了 `：` 却空着右边，看起来像"内容没加载出来"；光一个 `{级}级` 才是"这一级什么都没有"。
    //     （二选一里选了前者 —— 少一个符号，十行扫下来更整齐。）
    ...
    // 一行落在哪一态 → 取哪个颜色字段（判据就一条：跟当前级比 —— 到手 = 已解锁，没到手 = 未解锁）。
    private Color 逐级色(int 级, int 当前级)
        => 级 <= 当前级 ? 已获得色 : 未获得色;

    // 富文本着色：把一行文本包进 `<color=#RRGGBB>…</color>`。
    //   色值取自上面三个 `[SerializeField] Color`（Inspector 可调），这里只做 `Color` → `#RRGGBB` 的转换 ——
    //   代码里**没有颜色字面量**，换配色不用改源文件。
    //   ⚠ 前提：承载详情的 `TMP_Text` 必须开着 **richText**（TMP 默认开；本项目序列化过的 TMP 组件都是 `m_isRichText: 1`），
    //     否则这些标签会被当成可见文字原样画出来。
    private static string 着色(string 文本, Color 色)
        => $"<color=#{ColorUtility.ToHtmlStringRGB(色)}>{文本}</color>";

    // 一级行"解锁了什么"（本批用户定稿）：**只列三类结构化的东西**，一个字的长描述都不带。
    //   · 解锁技能[] → `解锁[名]`（多个用 `、` 连；**半角方括号** —— 本批用户口径：技能名一律用 `[ ]` 括）
    //   · 解锁配方[] → `配方：名`（已习得的加"（已习得）"—— 与 `书籍服务.发知识奖励` 同一判据）
    //   · 加成属性/加成值 → `暴击 +2%`（暴击/命中/闪避/医疗 按百分点、其余按点，见 `按百分点`）
    //   同类多项用 `、`、异类之间用全角空格（一行里把技能 / 配方 / 加成三段分开）；
    //   **空项一个字都不写**：某一级什么都没给 → 那一行只剩 `{级}级`（见 `逐级行文本`）。
    private static string 解锁文本(DataService 库, 玩家档案 玩家, 知识等级数据 行)
    {
        if (行 == null) return "";   // 表里缺这一级（数据没配全）：不写内容，也别崩
        var 段 = new List<string>();

        // 技能：只报"解锁了哪个"。上一版那句"（未到手）"的标注按本批口径去掉 ——
        //   用户要的是"这一级解锁什么"，"到没到手"是另一件事（`书籍服务.发知识奖励` 里属性前提不满足会记日志）。
