...
//     ⚠ **唯一的有意例外（本批用户定稿）**：逐级表的"三态"要用**富文本颜色**标注 → 本页为此多出
//       3 个 `[SerializeField] Color`（`已获得色 / 下一级色 / 未获得色`，默认值 = 档案板配色）
//       + `ColorUtility.ToHtmlStringRGB`（只把 `Color` 转成 `#RRGGBB`）。
//       为什么这仍不算破边界：**色值不在代码里**（三个字段在 Inspector 上可调，代码只做 Color → 十六进制的转换），
//       只是表达方式从"场景里摆一个预置节点"换成了"组件上的序列化字段"。除这三处外，本页与 `知识行` 的
...
public sealed class 知识子面板 : MonoBehaviour
{
    ...
    // —— 逐级表三态色（本批用户定稿；见类头"唯一的有意例外"）——
    // 为什么是**字段**而不是常量：颜色要能在 Inspector 上调；代码只管把 `Color` 转成 `#RRGGBB` 塞进富文本，
    //   所以"想换配色"不需要改代码（写死常量的话，用户每换一次色都得让程序改一遍源文件）。
    // 默认值 = 项目档案板配色（正文骨白 / 强调锈红 / 次要灰）。**每个值都比 hex 的精确商略高一丁点**：
    //   `ColorUtility.ToHtmlStringRGB` 内部是 `Color → Color32`，那一步是**截断**而不是四舍五入 ——
    //   写精确商（`228/255 ≈ 0.8941176`）会因为浮点误差落到 227，输出 `E3DFD5` 而不是 `E4DFD6`；
    //   这里每个分量都离截断边界 0.03/255 以上，算出来的 `#RRGGBB` 就稳定等于下面注释里的 hex。
    [SerializeField] private Color 已获得色 = new Color(0.895f, 0.875f, 0.840f);   // → #E4DFD6 正文骨白（228,223,214）
    [SerializeField] private Color 下一级色 = new Color(0.706f, 0.334f, 0.236f);   // → #B4553C 强调锈红（180, 85, 60）
    [SerializeField] private Color 未获得色 = new Color(0.542f, 0.514f, 0.471f);   // → #8A8378 次要灰（138,131,120）
    ...
    //         / `家族 · 领域` / `等级 VI / X` / `当前经验 a / b`（满级 = `已满级`）
    //   [块2] `物品数据.描述`
    //   [块3] 本条知识合计加成（**无加成时这一块整块不写**，不留一个空行）
    //   [块4] 逐级表（每级一行 `{级}级：{奖励}`，三态用**富文本颜色** —— 见 `逐级行文本`）
    // 为什么用**空行分块**：前几行是"一眼扫的当前状态"，描述与逐级表是"细读的"，中间空一行才分得开。
    // 为什么没选中就直接清空文本：详情只有一个节点，没有"半块要收起"的东西；空文本 = 什么都没显示，
    //   而 SetActive 反而不安全（用户可能把详情文本挂在页根附近，一关就把整页关掉）。
    // ⚠ 本批**删掉"持有 M 本"那一行**（用户定稿）：那是"背包里还躺着几本"的台账数，与"这门知识我练到哪"
    //   无关；摆在这一串里的第三行，还会被读成"升级要消耗 N 本"。连带**只服务于它**的取值方法
    //   `持有文本()` 一起删干净（没有第二个调用者，留着就是死代码）。
    ...
    // 逐级表：1..满级 每级一行，**三态用富文本颜色**（本批用户定稿，取代上一版的 `√ / → / ·` 前缀）——
    //   · 已获得（级 ≤ 当前级）→ `已获得色`；· 下一级（= 当前级 + 1）→ `下一级色`；· 未获得 → `未获得色`。
    ...
    private string 逐级行文本(DataService 库, 玩家档案 玩家, 物品数据 书, int 当前级)
    {
        ...
        for (int i = 1; i <= 满级; i++)
        {
            ...
            加行(文本, 着色(内容.Length == 0 ? $"{i}级" : $"{i}级：{内容}", 三态色(i, 当前级)));
        }
        return 文本.ToString();
    }

    // 一行落在哪一态 → 取哪个颜色字段（三态判据就是"和当前级比"）。
    private Color 三态色(int 级, int 当前级)
        => 级 <= 当前级 ? 已获得色 : (级 == 当前级 + 1 ? 下一级色 : 未获得色);
