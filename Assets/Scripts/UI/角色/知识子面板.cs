...
//     ⚠ 实测（本批开工时按 items_书籍.json 核过）：**13 本**（4 精通：近战/枪械/弓弩/医学 + 9 制作：初级/中级/高级/刀具/弓/防具/枪械制作）。
//       用户任务书写的是 11 本 → 见 `报书目数()`：数量不符只报一条 LogError，**不挡面板**、也不偷偷裁书。
...
public sealed class 知识子面板 : MonoBehaviour
{
    ...
    // 书目数核对：用户任务书写的是 11 本，实测数据是 13 本（多了 `盾精通` / `观察` —— 这两本**开局就可能已掌握**）。
    // 为什么只报错不裁书：数量对不上要么是任务书写旧了、要么是数据多配了，**这两种都不该由 UI 私自决定**——
    //   少显示一本 = 玩家"已掌握却查不到"；所以照全表显示，把差异喊出来让人去改数据或改口径。
    private void 报书目数()
    {
        if (列表.Count == 11) return;   // 口径一致（用户任务书里的 11 本）→ 不用出声
        Debug.LogError($"[知识子面板] 数据里「类型=书籍 且 书籍种类=知识」的书是 **{列表.Count} 本**，" +
                       "与设计口径的 11 本（4 精通 + 7 制作）不一致：" + string.Join("、", 列表) +
