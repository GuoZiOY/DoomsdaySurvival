...
//   · 右 = 详情区：**只有一个 `TMP_Text`**，所有细节按行拼进这一个文本（名称 / 家族 · 领域 / 等级 / 所需经验 /
//     持有 / 描述 / 本条合计加成 / 逐级表）。
...
//   · 经验**显示口径**（本批用户定稿："经验不是累计的，你的文本描述有问题 —— 改文本描述所需经验值即可"）：
//     页面上**一个字都不提"累计经验"**，只写这一档门槛本身（`所需经验 {门槛[当前级]}`，见 `经验文本`）；
//     底层语义**一个字都不动** —— `知识进度.经验` 仍是累计值、`成长管理器.加知识经验` 的升级判定仍是
//     `进度.经验 >= 门槛[级]`。改的只是"给人看的那句话"。
public sealed class 知识子面板 : MonoBehaviour
{
    ...
    // 详情拼法（用户定稿的顺序）：
    //   【块1】名称（品质着色富文本）/ `家族 · 领域` / `等级 VI / X` / `所需经验 T`（满级 = 已满级）/ `持有 M 本`
    //   【块2】`物品数据.描述`
    //   【块3】本条知识合计加成（**无加成时这一块整块不写**，不留一个空行）
    //   【块4】逐级表（每级一行，三态用前缀 + `· 所需 {门槛}` + 这一级解锁了什么 —— 见 `逐级行文本`）
    ...
    // 经验：**只写"所需经验"这一个数**（本批用户定稿）。
    //   为什么不再有分子分母：原写法 `累计经验 {经验} / {门槛}` 把**存起来的累计值**摆到人眼前，
    //   读起来就是"我要攒到 2000 才算完"—— 与玩家实际看到的进度不符（用户原话："经验不是累计的，这是错误的"）。
    //   现在这一行只回答一个问题："升到下一级还差多少"→ 就是这一档门槛本身（**下档门槛 = `知识门槛[当前级]`**）。
    //   未掌握（级 = 0）→ `知识门槛[0]`（= 升到 1 级要攒的累计经验）；满级 → `已满级`（没有下一档了）。
    //   ⚠ 这里**不读 `知识进度.经验`、也不做任何减法** —— 存储口径（累计）与升级判定（`成长管理器`）一个字都没动，
    //     本批只改"给人看的那句话"。
    private static string 经验文本(物品数据 书, int 级)
    {
        int 上限 = 级数(书);
    ...
    //   门槛仍取 `门槛取(书, i)`（= `知识门槛[i]`，与详情头部**同一个口径**：两处说的都是"下档门槛"）——
    //   行里写 `· 所需 {门槛}`（本批用户定稿：**避开"累计"字样**，与头部 `所需经验` 用同一个说法）；
    //   ⚠ **满级（i == 级数(书)）那一行不写门槛**（见循环里的注释）：没有"下一档"就不该再报一个 `所需`。
    //   本批删掉 `知识等级数据.描述`（用户口径：**只说明解锁什么，不要长描述**）——
    //   描述是给物品面板"读"的（数据里一课三十来个字），逐级表是"扫"的：十行挂上去就是一篇散文，
    //   读者反而找不到"我这一级到底拿到了什么"。取而代之的是 `解锁文本`（技能 / 配方 / 加成，全是结构化字段）。
    private static string 逐级行文本(DataService 库, 玩家档案 玩家, 物品数据 书, int 当前级)
    {
        ...
        for (int i = 1; i <= 满级; i++)
        {
            ...
            // **满级那一行不写门槛**（本批用户定稿）：`知识门槛` 是"升到**下一**级要攒多少"的口径，
            //   满级没有下一档 —— 照旧取 `门槛取` 出来的只是上一格的值（`· 10 级 · 所需 2000` 与 9 级同数），
            //   读起来像"到了 10 级还要再攒 2000"，是句假话。于是满级行只留 `{前缀} {i} 级` + 解锁内容。
            // ⚠ 1..满级-1 行**一个字都不动**：门槛仍是 `门槛取(书, i)`（= `知识门槛[i]`，与详情头部同一个口径）。
            string 门槛段 = i >= 满级 ? "" : $" · 所需 {门槛取(书, i)}";
            加行(文本, $"{前缀(i, 当前级)} {i} 级{门槛段}" + (内容.Length == 0 ? "" : "　" + 内容));
        }
        return 文本.ToString();
    }
    ...
    //   同类多项用 `、`、异类之间用全角空格（与"所需 {门槛}"后面那个分隔符同款）；
    //   **空项一个字都不写**：某一级什么都没给 → 那一行只到门槛为止（用户口径：`{前缀} {级} 级 · 所需 {门槛}`）。
