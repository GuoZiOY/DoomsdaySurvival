...
public sealed class 战斗面板 : 面板基类
{
    ...
    [SerializeField] private Button 返回行动按钮;
    [SerializeField] private Button 取消按钮;             // 选目标时的"取消"（初始隐藏，允许反悔）
    ...
    private readonly List<GameObject> 子菜单项 = new List<GameObject>();
    ...
    void Awake()
    {
        ...
        返回行动按钮.onClick.AddListener(回主行动);
        取消按钮?.onClick.AddListener(回主行动);
    ...
    private void 显示子菜单(bool 是技能)
    {
        ...
        if (是技能)
        {
            foreach (var 标识 in 战斗.玩家.已学技能)
            {
                ...
                var 标识副本 = 标识;
                创建菜单项($"{技能.名称}（{技能.消耗魔力}MP）", () => 开始选目标(标识副本));
            }
        }
        else
        {
    ...
    private void 创建菜单项(string 文字, System.Action 点击)
    {
        var 按钮 = 面板基类.按钮预制体 != null
            ? Instantiate(面板基类.按钮预制体, 列表容器, false)
            : null;
        if (按钮 == null) return;
        按钮.SetActive(true);
        var 文本 = 按钮.transform.Find("文字")?.GetComponent<TMP_Text>();
        if (文本 != null) 文本.text = 文字;
        var 组件 = 按钮.GetComponent<Button>();
        if (组件 != null)
        {
            组件.onClick.AddListener(() => 点击());
            音效管理器.实例?.注册按钮(组件);   // 战斗子菜单项动态按钮成功音效
        }
        子菜单项.Add(按钮);
    }
    ...
    private void 开始选目标(string 行动)
    {
        ...
        子菜单区?.gameObject.SetActive(false);
        主行动区.gameObject.SetActive(false);
        取消按钮?.gameObject.SetActive(true);   // 选目标时可反悔
