...
public abstract class 面板基类 : MonoBehaviour
{
    ...
    // 这个面板现在能不能被"HUD 上的关闭按钮"关掉（决定那颗按钮显不显示）。
    // 判据 = 面板**覆写了 `取消文本`** —— 覆写它就意味着它有可执行的退出口（见各面板的 `回退()`）。
    // 这样不用给每个面板加一个标志位、也不会漏（战斗沙盒面板没覆写 → 战斗里那颗按钮自动不出现）。
    // 代价是一条要守住的约定：**覆写了文案，就必须让 `回退()` 真的能退出去**。
    // 将来若有"有文案但不该被外面关掉"的面板，覆写本属性返回 false 即可。
    public virtual bool 可关闭 => 取消文本 != "取消";
