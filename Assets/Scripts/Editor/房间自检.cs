...
public static class RoomSelfCheck
{
    ...
    public static void 自检(int 种子数, bool 记日志)
    {
        ...
        int 失败 = 0, 总数 = 0, 保底 = 0, 容器总 = 0, 敌人总 = 0, 门总 = 0;
        foreach (var 模板 in 数据.房间模板.Values)
            for (int 种子 = 1; 种子 <= 种子数; 种子++)
            {
                ...
                {
                    var a = 甲.实体[i];
                    if (a == null || a.类型 == 网格实体类型.墙 || a.是玩家) continue;
                ...
                // 门：外墙上真的开了洞 + 位置约束 + 内门数量对得上
                var 门们 = 甲.门列表();
                var 大门 = 甲.大门();
                if (大门 == null) 失败 += 报($"{模板.标识} 种子{种子}：没有大门");
                else
                {
                    var 内 = 甲.门内侧格(大门);
                    if (内.列 != 甲.入口列 || 内.行 != 甲.入口行)
                        失败 += 报($"{模板.标识} 种子{种子}：大门不在入口格外侧");
                }
                int 声明内门 = 模板.门?.Length ?? 0, 实际内门 = 0;
                foreach (var 门 in 门们) if (!门.是大门) 实际内门++;
                if (实际内门 != 声明内门) 失败 += 报($"{模板.标识} 种子{种子}：内门 {实际内门} 扇 ≠ 声明 {声明内门} 扇");
                if (甲.门开不出 != 0) 失败 += 报($"{模板.标识} 种子{种子}：{甲.门开不出} 扇门排不开");
                for (int i = 0; i < 门们.Count; i++)
                {
                    var 门 = 门们[i];
                    // 无锁的门 = 门洞（不占格、不挡视线、能站）；锁着的门 = 关着的门（占格 + 挡视线，只能站在门前）
                    var 格上 = 甲.占位(门.列, 门.行);
                    if (!门.锁着 && 格上 != null)
                        失败 += 报($"{模板.标识} 种子{种子}：门格上还压着墙（没开洞）{门.标识}@{门.列},{门.行}");
                    if (门.锁着 && 格上 != 门)
                        失败 += 报($"{模板.标识} 种子{种子}：锁着的门没占住自己那格 {门.标识}");
                    if (甲.可通行(门.列, 门.行) == 门.锁着)
                        失败 += 报($"{模板.标识} 种子{种子}：门的可通行性与锁态不符 {门.标识}");
                    if (门.锁着 != 门.挡视线)
                        失败 += 报($"{模板.标识} 种子{种子}：门的挡视线与锁态不符 {门.标识}");
                    if (!门.锁着 && 门.占格)
                        失败 += 报($"{模板.标识} 种子{种子}：没锁的门不该占格 {门.标识}");
                    bool 在外墙 = 门.列 == 0 || 门.行 == 0 || 门.列 == 甲.列 - 1 || 门.行 == 甲.行 - 1;
                    if (!在外墙) 失败 += 报($"{模板.标识} 种子{种子}：门不在外墙上 {门.标识}@{门.列},{门.行}");
                    int 期望边 = 门.行 == 甲.行 - 1 ? 0 : (门.列 == 0 ? 1 : (门.行 == 0 ? 2 : 3));
                    if (门.朝向 != 期望边) 失败 += 报($"{模板.标识} 种子{种子}：门朝向与所在边不符 {门.标识}");
                    bool 竖边 = 门.朝向 == 0 || 门.朝向 == 2;
                    int 号 = 竖边 ? 门.列 : 门.行, 长 = 竖边 ? 甲.列 : 甲.行;
                    if (号 < 房间生成器.门离角 || 号 > 长 - 1 - 房间生成器.门离角)
                        失败 += 报($"{模板.标识} 种子{种子}：门贴墙角 {门.标识} 第{号}格");
                    var 内侧 = 甲.门内侧格(门);
                    if (!甲.可通行(内侧.列, 内侧.行) || !可达.Contains(网格数据.编码(内侧.列, 内侧.行)))
                        失败 += 报($"{模板.标识} 种子{种子}：门内侧格走不到 {门.标识} → {内侧.列},{内侧.行}");
                    if (!门.是大门)
                    {
                        if (string.IsNullOrEmpty(门.通向)) 失败 += 报($"{模板.标识} 种子{种子}：内门没写通向 {门.标识}");
                        else if (!数据.房间模板.ContainsKey(门.通向)) 失败 += 报($"{模板.标识} 种子{种子}：门通向的房间不存在 {门.通向}");
                        // 门锁：钥匙物品得存在，而且这间房得有地方放钥匙（柜子 / 敌人尸体）
                        if (!string.IsNullOrEmpty(门.锁钥匙))
                        {
                            if (!数据.物品.ContainsKey(门.锁钥匙))
                                失败 += 报($"{模板.标识} 种子{种子}：门的钥匙物品不存在 {门.锁钥匙}");
                            if (甲.容器总数() == 0 && 甲.敌人总数() == 0)
                                失败 += 报($"{模板.标识} 种子{种子}：门锁着但这间房既没容器也没敌人（钥匙无处可放）{门.标识}");
                        }
                    }
                    for (int j = i + 1; j < 门们.Count; j++)
                    {
                        ...
                        if (Math.Abs(他号 - 号) < 房间生成器.门间距)
                            失败 += 报($"{模板.标识} 种子{种子}：同边两扇门太近 {门.标识} / {门们[j].标识}");
                    }
                }
                // 含大门=false（从门进来的那间房）：大门必须没有，内门数量照旧
                var 无大门 = 房间生成器.生成(种子, 模板, (地图, 房间) => 取容器表(数据, 地图, 房间), id => 取敌人组(数据, id), false);
                if (无大门?.大门() != null) 失败 += 报($"{模板.标识} 种子{种子}：含大门=false 却生成了大门");

                // 视野
                var 可见 = 网格视野.可见集(甲, 甲.入口列, 甲.入口行, 甲.视野边长);
                if (可见.Count == 0 || !可见.Contains(网格数据.编码(甲.入口列, 甲.入口行)))
                    失败 += 报($"{模板.标识} 种子{种子}：入口视野异常");
            }

        string 汇总 = $"[自检] 房间层：生成 {总数} 次（{数据.房间模板.Count} 模板 × {种子数} 种子），" +
                      $"保底布局 {保底}，平均容器 {(总数 == 0 ? 0 : (float)容器总 / 总数):F1} / 敌人 {(总数 == 0 ? 0 : (float)敌人总 / 总数):F1}" +
                      $" / 门 {(总数 == 0 ? 0 : (float)门总 / 总数):F1}，失败 {失败}";
    ...
    private static bool 有相邻可达格(网格数据 房, HashSet<int> 可达, 网格实体 e)
    {
        for (int r = e.行; r < e.行 + e.高; r++)
            ...
            {
                if (可达.Contains(网格数据.编码(c - 1, r))) return true;
                if (可达.Contains(网格数据.编码(c + 1, r))) return true;
                if (可达.Contains(网格数据.编码(c, r - 1))) return true;
                if (可达.Contains(网格数据.编码(c, r + 1))) return true;
            }
        return false;
    }
