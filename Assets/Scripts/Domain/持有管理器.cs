...
// 仓库视图/穿戴容器视图 包装（复用 背包服务 网格算法）。
// 数据（装备容器物品/仓库/背包服务/解析委托）由 玩家档案 持有；玩家档案 门面转发调用。
public sealed class 持有管理器
{
    public readonly 玩家档案 玩家;
    public 持有管理器(玩家档案 玩家) { this.玩家 = 玩家; }

    // —— 旧主背包 网格数据（废弃：不再存放游戏物品；保留 网格算法 与 旧引用 兼容——网格面板 数据源 null 时显示空网格兜底） ——
    public List<物品堆叠> 背包 => 玩家.背包服务.背包;
    public int 网格列 { get => 玩家.背包服务.网格列; set => 玩家.背包服务.网格列 = value; }
    public int 网格行 { get => 玩家.背包服务.网格行; set => 玩家.背包服务.网格行 = value; }
    ...
    // 公开：指定 穿戴容器槽位（弹挂/腰封/背包）的 背包服务 视图（卸下回原容器用；槽空/未穿戴 = null）
    public 背包服务 穿戴容器视图(string 槽位)
    {
    ...
    // 临时 穿戴容器视图（背包服务 包装 容器物品 + 注入解析器）
    private 背包服务 穿戴容器服务(装备记录 记录)
    {
        var 服务 = new 背包服务();
    ...
    // 仓库视图：把 仓库物品 包成 背包服务（复用全部网格/拖拽/跨网格转移逻辑）
    public 背包服务 仓库视图()
    {
        var 服务 = new 背包服务();
        服务.背包 = 玩家.仓库物品;
        服务.网格列 = 玩家.仓库列;
        服务.网格行 = 玩家.仓库行 + 玩家.家具效果("储物箱");   // 储物箱 家具：升级 扩仓库 行数（2/4/8；未建=0）
        服务.形状解析 = 玩家.形状解析;
        服务.堆叠上限解析 = 玩家.堆叠上限解析;
        服务.有效最大耐久解析 = 标识 => 玩家.装备管理.有效最大耐久(标识);
        服务.重量解析 = 玩家.重量解析;
