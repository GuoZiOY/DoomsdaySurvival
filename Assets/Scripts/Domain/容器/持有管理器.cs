...
// 负责：所有持有物品 查询（含嵌套容器递归）；物品数量/移除/配件/耐久；统一放入（穿戴容器优先 → 仓库兜底）；
...
public sealed class 持有管理器
{
    ...
    public List<配件条> 背包配件(string 标识)
    {
        foreach (var 堆叠 in 所有持有物品())
            if (堆叠.标识 == 标识 && 堆叠.数量 > 0) return 堆叠.配件;
        return null;
    }
    ...
    // 背包里某标识的**已装填**（刀44：换枪时把弹匣里的子弹一起带走）
    public int 背包已装填(string 标识)
    {
        foreach (var 堆叠 in 所有持有物品())
            if (堆叠.标识 == 标识 && 堆叠.数量 > 0) return 堆叠.已装填;
        return 0;
    }
    ...
    // 放入具体堆叠实例（带配件/容器数据）：穿戴容器优先 → 仓库兜底；已在某网格中 返回 true（不移动）
