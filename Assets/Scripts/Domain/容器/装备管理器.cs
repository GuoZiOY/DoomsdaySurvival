...
public sealed class 装备管理器
{
    ...
    // 副属性（装备本体 + 配件 + 知识）：**命中 / 暴击 / 闪避 / 潜行 / 速度 / 生命上限** 全从这里取
    //   → 知识加成只在这一处接一次，上面那些派生属性就都自动吃到了（所以它们内部不再单独加知识加成）。
    public int 副属性(加成类型 类) => 本体副属性(类) + 配件加成(类) + 知识加成(类);

    // ================= 装备驱动 派生数值 =================

    // 知识加成（书籍·知识书：精通/制作）：遍历 已掌握知识，累加「已解锁等级」里 加成属性 与该类同名 的 加成值。
    // 与 装备/配件 走同一条派生链 —— 知识是**永久被动**，不占装备位、不占技能槽（与 天赋 / 被动技能 并列的第三条边）。
    // 代价：每次读派生属性都要遍历（知识 ≤ 11 条 × 等级表 ≤ 10 行），量级很小，且只在 面板刷新 / 战斗生成 时读。
    public int 知识加成(加成类型 类)
    {
        if (玩家.已掌握知识 == null || 玩家.知识等级解析 == null) return 0;
        string 名 = 类.ToString();
        int 总 = 0;
        foreach (var 进度 in 玩家.已掌握知识)
        {
            if (进度 == null || 进度.等级 <= 0) continue;
            var 表 = 玩家.知识等级解析(进度.标识);
            if (表 == null) continue;
            foreach (var 行 in 表)
                if (行 != null && 行.级 <= 进度.等级 && 行.加成属性 == 名) 总 += (int)行.加成值;
        }
        return 总;
    }

    // 负重上限：50 + 力量×5 + 体质×2 + 装备/配件 + 知识
    public int 负重上限 => 50 + 玩家.力量 * 5 + 玩家.体质 * 2 + 装备数值(玩家.负重加成解析) + 配件加成(加成类型.负重) + 知识加成(加成类型.负重);
    ...
    // 天赋「战斗直觉」（退役军人）：攻击力 +20% —— 乘在下面**两个各算各的**攻击派生值上（近战 / 枪械，各一次）。
    // 为什么"两处各乘一次"不等于"同一个基础乘两遍"：近战伤害 = 力量+装备+配件+知识−骨折，
    //   枪械伤害 = 装备+配件+知识（不吃力量）——两条独立算式，同一次攻击由 `伤害计算.计算` 按 伤害类型
    //   **二选一**（近战攻击取 当前近战、远程攻击取 当前远程），所以一次攻击只会吃到其中一份 ×1.2。
    // 与「肾上腺素」（攻击 +20% buff）的关系：本天赋改的是"基础攻"（这里），buff 在 `战斗单位.当前近战/当前远程`
    //   上再乘一次 → **乘算叠加**（两次各 20%，≈1.44 倍），不走 `防御减伤` 那条加算链。
    // 取整：`(int)(值 × 1.2f)` 截断 —— 与 Domain 里其他天赋乘率（成长管理器·学霸 ×1.15）同口径。
    private int 天赋战斗直觉(int 值) => 玩家.天赋.Contains("战斗直觉") ? (int)(值 * 1.2f) : 值;

    // 近战伤害：力量 + 武器/配件 + 知识（骨折削弱）
    public int 近战伤害 => Math.Max(1, 天赋战斗直觉(玩家.力量 + 装备数值(玩家.攻击加成解析) + 配件加成(加成类型.攻击) + 知识加成(加成类型.攻击) - (玩家.骨折 > 0 ? 玩家.骨折 / 10 : 0)));

    // 枪械伤害：武器固定伤害（不吃属性）+ 配件 + 知识
    public int 枪械伤害 => 天赋战斗直觉(装备数值(玩家.攻击加成解析) + 配件加成(加成类型.攻击) + 知识加成(加成类型.攻击));

    // 总防御：体质×0.5 + 防具/配件 + 知识
    public int 总防御 => 玩家.体质 / 2 + 装备数值(玩家.防御加成解析) + 配件加成(加成类型.防御) + 知识加成(加成类型.防御);

    // 暴击率%：敏捷×1 + 意志×0.5 + 副属性（含配件与知识）（封顶 80）
    public float 暴击概率 => Math.Min(0.8f, (玩家.敏捷 * 1f + 玩家.意志 * 0.5f) / 100f + 副属性(加成类型.暴击) / 100f);

    // 闪避率%：敏捷×1 + 意志×0.3 + 副属性（含配件与知识）（封顶 60）
    public float 闪避概率 => Math.Min(0.6f, (玩家.敏捷 * 1f + 玩家.意志 * 0.3f) / 100f + 副属性(加成类型.闪避) / 100f);

    // 潜行值：敏捷×2 + 意志×1 + 副属性（含配件与知识）（躲避丧尸/偷袭判定）
