...
public sealed class 装备管理器
{
    ...
    public 装备记录 装备到槽(string 槽位, 装备记录 记录)
    {
        ...
        foreach (var e in 玩家.装备)
            if (e.槽位 == 槽位)
            {
                var 旧 = new 装备记录(槽位, e.标识) { 配件 = e.配件, 当前耐久 = e.当前耐久, 容器列 = e.容器列, 容器行 = e.容器行, 容器物品 = e.容器物品, 来源 = e.来源 };
                e.标识 = 记录.标识;
                e.配件 = 记录.配件;
                e.当前耐久 = 记录.当前耐久;
                e.容器列 = 记录.容器列;
    ...
    public 装备记录 装备到槽(string 槽位, string 标识, List<配件条> 配件 = null, int? 当前耐久 = null, 物品堆叠 容器源 = null, string 来源 = null)
    {
        ...
        var 新 = new 装备记录(槽位, 标识) { 配件 = 配件, 当前耐久 = 当前耐久 ?? 有效最大耐久(标识), 来源 = 来源 };
    ...
    // ⚠ v51 刀41 修：**配件（改装件）不吃这个兜底**。
    //   原来 陶瓷插板（防御加成 +6、JSON 里没写 最大耐久）会被兜底成 15 → 谁要是建堆叠时忘了初始化
    //   当前耐久（默认 0），玩家看到的就是"拆出来的插板是**损坏**的"（用户实测报的就是这个）。
    //   语义上也对：配件是改装件，磨不磨损由 items.json 说了算，不该被武器/防具的兜底规则牵连。
    public int 有效最大耐久(string 标识)
    {
        int 配 = 玩家.最大耐久解析?.Invoke(标识) ?? 0;
        if (配 > 0) return 配;
        if (玩家.物品类型解析?.Invoke(标识) == "配件") return 0;   // 配件：只有显式配了 最大耐久 才有耐久
