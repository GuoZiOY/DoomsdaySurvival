...
// ============================================================
// 网格视野：以玩家为中心、**边长是单数（3/5/7/9…）的正方形**可见区（纯 C#，零 UnityEngine 依赖）。
...
public static class 网格视野
{
    ...
    // 以 (列,行) 为中心、边长 为单数 的正方形可见集（元素 = 网格数据.编码），越界部分裁掉。
    public static HashSet<int> 可见集(网格数据 房, int 列, int 行, int 边长)
    {
        ...
        for (int 行2 = 行 - 半; 行2 <= 行 + 半; 行2++)
            for (int 列2 = 列 - 半; 列2 <= 列 + 半; 列2++)
                if (房.界内(列2, 行2)) 集.Add(网格数据.编码(列2, 行2));
        return 集;
    }
    ...
    public static bool 视线通(网格数据 房, int 起列, int 起行, int 止列, int 止行)
    {
