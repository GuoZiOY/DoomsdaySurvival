...
public sealed class 战斗单位
{
    ...
    // —— 受伤乘数（天赋「铁骨」这类"整条伤害的最终减免"，默认 1 = 不减） ——
    // 为什么必须是**独立乘数**而不是并进 `防御减伤`：物防 1 点 = 1% 的加算链（物防 50% + 抗性 50% + …）
    //   再多一层 20% 很快就滚到"打不动"；这里是**在伤害管线的出口整段乘 0.8**（乘算，永远减不到 0）。
    // 只有玩家单位会被置成非 1（`从玩家投影` 按天赋写）；敌人恒 1（`从敌人生成` 不碰这个字段）。
    public float 受伤乘数 = 1f;

    // —— 无敌（天赋触发后：免疫伤害一轮；一轮 = 5 现实秒，递减在 BattleService.轮次结算） ——
    ...
    // （行动 频率 改由 意图间隔秒 决定，见 移动/攻击/技能间隔秒；基础速度 供 当前速度（吃 速度 buff）与 逃跑 对比 读）
    ...
    // —— 被动技能（我方投影：装在 战斗技能槽 上的被动，与主动混装 6 格；开局自动生效；不进 `已学技能` 故不可点击释放） ——
    public readonly HashSet<string> 被动技能 = new HashSet<string>();
    ...
    // 敏捷 buff（猎人直觉）→ 速度/暴击/闪避 同步提升；**速度 buff（专注）只进速度这一条**
    //   （用户口径：速度 = 敏捷×2 + 配件，速度 buff 只影响行动条填充，不加暴击/闪避）
    public int 当前速度 => 修正(基础速度, 属性修正("敏捷") + 属性修正("速度"));
    ...
    // 持续伤害量（异常且非控制 buff 的**每 5 秒**伤害总和）；时机："开始"=本轮结算先扣（缺省）/ "结束"=本轮结算末扣
    ...
    // buff 计时递减：**现实秒**（即时制）；到 0 移除。
    //   为什么按秒而不是按"出手次数"：时长数据（buffs.json `持续秒`）就是秒，
    //   而同一场里各单位出手次数各不相同 —— 按出手次数递减会让"15 秒流血"在快慢单位身上长短不一。
    //   结算（持续伤害/再生）由 BattleService 每 5 现实秒 调一次，这里只管时长。
    public void 推进Buff(float 现实秒)
    {
        for (int i = Buffs.Count - 1; i >= 0; i--)
        {
            var b = Buffs[i];
            b.剩余秒 -= 现实秒;
            if (b.剩余秒 <= 0f) Buffs.RemoveAt(i);
        }
    }
    ...
    public static 战斗单位 从玩家投影(玩家档案 玩家)
    {
        var 单位 = new 战斗单位
        {
            ...
            // 天赋「铁骨」（消防员）：受到的伤害 −20%。
            //   · **只对玩家**：这一句在 `从玩家投影` 里，敌人走 `从敌人生成`（不碰该字段，恒 1）；
            //   · **乘算**：出口整段乘 0.8，不与 `防御减伤` 那条加算链混在一起（见字段注释）；
            //   · **走唯一管线**：只有 `BattleService.造成伤害` 的出口读它 —— 不在别处再减一次。
            受伤乘数 = 玩家.天赋.Contains("铁骨") ? 0.8f : 1f
        };
        // 玩家技能：战斗内可用 = 战斗技能槽（固定 6 槽）内技能（学更多技能 → 槽满则不进战斗）
        // 主动/被动混装（用户 2026-09-17 拍板 C）：被动 分流进 `被动技能`，不进 `已学技能`（否则会被列入可释放技能）
        foreach (var 槽标识 in 玩家.战斗技能槽 ?? new List<string>())
        {
            if (string.IsNullOrEmpty(槽标识)) continue;
            var 槽技能 = 玩家.技能解析?.Invoke(槽标识);
        ...
        // 被动·增幅：开局一次性叠到 战斗属性（"机制"类 不叠属性 —— 由战斗逻辑按标识查询，见 数据模型.技能数据.被动类型）
        单位.应用被动增幅(玩家);
    ...
    // 被动·增幅：遍历 装在槽里的被动，逐条叠加到 战斗属性（"机制"类 不在这里 —— 由战斗逻辑按标识查询）
    private void 应用被动增幅(玩家档案 玩家)
    {
        foreach (var 标 in 被动技能)
        {
            var 技 = 玩家.技能解析?.Invoke(标);
            if (技 == null || 技.被动类型 != "增幅") continue;
            叠加增幅(技.增幅属性, 技.增幅单位, 技.增幅值);
        }
    }

    // 单条增幅：数值类 "点" 直加 / "%" 按比例放大；率类（暴击·命中·闪避）固定按 百分点 直加，上限沿用既有口径
    private void 叠加增幅(string 属性, string 单位, float 值)
    {
        if (string.IsNullOrEmpty(属性) || 值 == 0f) return;
        bool 按比例 = 单位 == "%";
        switch (属性)
        {
            case "近战": 基础近战 = 数值加(基础近战, 值, 按比例); break;
            case "远程": 基础远程 = 数值加(基础远程, 值, 按比例); break;
            case "防御": 基础物防 = 数值加(基础物防, 值, 按比例); break;
            case "暴击": 暴击率 = 封顶(暴击率 + 值 / 100f, 0.8f); break;
            case "命中": 命中率 = 封顶(命中率 + 值 / 100f, 0.99f); break;
            case "闪避": 闪避率 = 封顶(闪避率 + 值 / 100f, 0.8f); break;
            case "生命上限":
                int 生命增 = 数值加(最大生命, 值, 按比例) - 最大生命;
                最大生命 += 生命增; 生命 += 生命增; break;
            case "精力上限":
                int 精力增 = 数值加(最大精力, 值, 按比例) - 最大精力;
                最大精力 += 精力增; 精力 += 精力增; break;
        }
    }

    // 数值类增幅：按比例 → 原值×(1+值/100)；否则 → 原值 + 值
    private static int 数值加(int 原值, float 值, bool 按比例) => 按比例 ? (int)(原值 * (1f + 值 / 100f)) : 原值 + (int)值;

    // 率类封顶（暴击/闪避 0.8、命中 0.99，与 `当前暴击率` / `命中率公式` 同口径）
    private static float 封顶(float 值, float 上限) => 值 > 上限 ? 上限 : 值;
...
{
    public Buff定义 定义;
    public float 剩余秒;   // 剩余**现实秒**（即时制；由 战斗单位.推进Buff 递减）
    public int 层数;
}
