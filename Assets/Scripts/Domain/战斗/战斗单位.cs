...
public sealed class 战斗单位
{
    // —— 身份 ——
    public string 名称;
    public List<string> 词缀名;   // 敌人的词缀**名字**（v51 刀45；刀48 起只存名字如"强壮"，显示由状态行包成 [强壮]；玩家恒 null）
    ...
    // 近战攻击（刀55）：决定这次普攻走**物理**（吃力量加成）还是**远程**（固定伤害）。
    //   **不再看 `攻击距离 > 1`** —— 长柄近战射程 2，照样是近战。
    //   玩家按当前武器种类判；敌人暂按"射程 ≤ 1 = 近战"（敌人数据里没有武器种类）。
    public bool 近战攻击 = true;
    ...
    // 玩家命中率 = 基础 95% + 命中配件（v51 刀35）
    // 为什么补这一条：`加成类型.命中` 一直是"武器抽得到、数值也进存档、但**没有任何地方读**"的空词条 ——
    //   玩家看到"命中 +3"却毫无效果（静默骗人）。这里把它接进命中判定（`伤害计算.是否命中` 的第一个参数）。
    // 上限 0.99：留 1% 天然失手，也避免溢出后出现"闪避再高也必中"的怪状态。
    private static float 命中率公式(玩家档案 玩家)
    {
        float 值 = 0.95f + 玩家.装备管理.副属性(加成类型.命中) / 100f;
        return 值 > 0.99f ? 0.99f : 值;
    }
    ...
    public static 战斗单位 从玩家投影(玩家档案 玩家)
    {
        var 单位 = new 战斗单位
        {
            ...
            基础速度 = 玩家.速度,              // 速度 = 敏捷×2 + 装备速度配件
            基础敏捷 = 玩家.敏捷,              // 敏捷 buff 作用于速度/暴击/闪避
            暴击率 = 玩家.暴击概率,
            闪避率 = 玩家.闪避概率,
            命中率 = 命中率公式(玩家),       // 0.95 + 命中加成（装备本体 + 配件，见 命中率公式）
        ...
        单位.近战攻击 = 武器种类判定.是近战(单位.当前武器);   // 刀55：近战/远程按武器种类（长柄射程 2 仍是近战）
        return 单位;
    }
    ...
    public static 战斗单位 从敌人生成(敌人数据 敌人)
    {
        ...
        单位.近战攻击 = 单位.攻击距离 <= 1;   // 敌人没有武器种类字段：射程 ≤ 1 视为近战（吃力量不变）
        单位.移动AI = 敌人.移动AI;
