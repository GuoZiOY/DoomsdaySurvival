...
using UnityEngine;
    ...
    public sealed class DataService
    {
        ...
        private static readonly HashSet<string> 合法物品类型 = new HashSet<string>
        {
            ...
            "配件",   // 改装件（v51 刀35 起取代装备随机词缀）：装在装备配件槽上的真实物品
        };
        ...
        private void 加载物品()
        {
            ...
            加载("物品/items_配件", 物品, (物品根 根) => 根.物品);   // 配件（改装件）：装在装备配件槽上的真实物品
        ...
        public void 重新校验()
        {
            ...
            foreach (var (标识, 特) in 敌人特性表)
            {
                ...
                foreach (var 效 in 特.效果们)
                {
                    if (效 == null) continue;
                    if (!敌人特性效果名.认得出(效.效果))
                        校验错误.Add($"敌人特性[{标识}] → 效果[{效.效果}] 认不出（只能是 {string.Join("/", 敌人特性效果名.全部)}）");
                    if (效.数值 == 0)
            ...
            foreach (var (标识, 物品) in 物品)
            {
                ...
                // —— 配件（改装件，v51 刀35）：槽位必须认得 + 至少要有一条加成 ——
                //   为什么要查：配件的效果**只**来自物品自己的加成字段；槽位写错 = 永远装不上，
                //   一条加成都没有 = 装上也没用 —— 两种都是"看起来有这件东西，实际什么也不干"的静默骗人。
                if (物品.类型 == "配件")
                {
                    if (!配件槽.认得出(物品.槽位))
                        校验错误.Add($"配件[{标识}] → 槽位[{物品.槽位}] 非法（只能是 {string.Join("/", 配件槽.全部)}）");
                    if (配件加成.全部(物品).Count == 0)
                        校验错误.Add($"配件[{标识}] 没有任何加成（装上等于没用 —— 至少给一条 攻击/防御/命中/潜行…）");
                    // 配件不是装备、**没有耐久**（用户 2026-09-13 定；`装备管理器.有效最大耐久` 里也是硬规则）
                    //   → 数据里配了 最大耐久 就是写错了，必须报出来，别让它静静地看起来像"会用坏"
                    if (物品.最大耐久 > 0)
                        校验错误.Add($"配件[{标识}] 配了 最大耐久={物品.最大耐久} —— 配件不是装备、没有耐久，请删掉这个字段");
                    // 弹匣容量加成 只对**弹匣槽**配件有意义（其他槽的配件写了它 = 装了也不进容量，静默无效）
                    if (物品.弹匣容量加成 != 0 && 物品.槽位 != 配件槽.弹匣)
                        校验错误.Add($"配件[{标识}] 配了 弹匣容量加成 但槽位是[{物品.槽位}] —— 只有 弹匣 槽的配件才能改容量");
                }
                // —— 弹匣（刀44）：只做枪械（手枪/步枪/霰弹枪）；弓弩与近战不许配 弹匣容量 ——
                if (物品.类型 == "武器" && 物品.弹匣容量 != 0)
                {
                    bool 是枪械 = 物品.武器种类 == "手枪" || 物品.武器种类 == "步枪" || 物品.武器种类 == "霰弹枪";
                    if (!是枪械)
                        校验错误.Add($"武器[{标识}] 配了 弹匣容量={物品.弹匣容量} 但武器种类是[{物品.武器种类}] —— 弹匣只做枪械（弓弩/近战 不走弹匣）");
                    if (物品.弹匣容量 < 1)
                        校验错误.Add($"武器[{标识}] 弹匣容量={物品.弹匣容量} 非法（要么不配=无弹匣，要么 >= 1）");
                }
            }
            ...
            foreach (var (标识, 世) in 世界)
            {
                ...
                if (世.搜索 != null)
                {
                    ...
                    //   那样临时建筑照样能拉出抗生素/军用医疗包。这条把"奖励越级"从数据层面彻底焊死。
                    if (世.搜索.临时建筑 != null && 世.搜索.临时建筑.Length > 0)
                    {
