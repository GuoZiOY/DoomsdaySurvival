using System;
    ...
    //   承重件没丢：解锁门禁改由 `解锁条件项`（天数/物品/前置区域）承担，落点由 `世界模板.营地` 承担。
    ...
    public class 区域模板
    {
        ...
        // —— 副本化（v51）——
        public int 重置小时 = 0;          // 副本内容重置周期（游戏小时）；0 = 永不重置（一次性副本）
        public int 稀有度 = 1;            // 资源档 1 普通 / 2 较好 / 3 稀有 —— 与"临时建筑只给普通资源"区分（数据表达，不做代码分支）
    }
    ...
    //   条件 = "情报"（持有某条情报）/ "物品"（持有某物品）/ "天数"（活到第 N 天）/ "前置区域"（已通关某片区域）
    //   注意：**没有"主线阶段"** —— 那套奇幻章节（主线阶段.cs 的 镇长/橡木林/风谷村/骑士团）随 v51 一起清掉，
    //        末日版的进度门禁就是这里的四条，够用且不引奇幻概念。
    [Serializable]
    public class 解锁条件项
    {
        public string 条件;              // "情报" / "物品" / "天数" / "前置区域"
        public string 标识;              // 情报 标题 / 物品标识 / 前置区域标识（"天数" 不用这个）
