using System;
    ...
    //   （如 物品定义.品质档、伤病类型 字段），域层（品质工具.倍率 / 生存管理器）也要读。
    ...
    // ================= 剧情【整条线已删】 =================
    // 原来这里有 `剧情效果` / `物品获得项` / `物品失去项` / `剧情选项` / `剧情节点` / `剧情根` 六个 DTO，
    // 以及 `Services/剧情/DialogueService.cs` + `Events/剧情事件.cs` + `UI/功能面板/对话面板.cs`
    // + `Domain/玩家/效果结算.cs`（剧情效果结算）那一整套。
    // 用户定稿：**对话/剧情不做，以后也不会做** → 本批整条删除。
    // 取证：`story.json` 从来没有这个文件（数据从没落地过）；`效果结算` 全仓库 0 调用（唯一消费者是 DialogueService）。
    //
    // 原有一个 `区域剧情路由`（进入某地点时按当前主线阶段自动触发剧情）—— 已随 v51 刀7b 删：
    // 它的**唯一消费者**是 地图服务.自动触发剧情，而 地图服务 在刀3 就退役了；
    // 它挂在 `主线阶段` 上，而主线阶段本身已删。
    ...
    // （DialogueService 自己也在"对话/剧情整条线不做"那一批里删掉了 —— 上面两条是当时取证的记录。）
