using System;
    ...
    public class 物品数据
    {
        ...
        // 读透被动：这本书 被 读透（耐久 归零 崩解）时 **必定** 授予 的 被动技能标识（可空）。
        // 为什么不放进 `技能池`：指南 走 池抽 1 条（已学会 降权 ×0.2），被动 落进池里 就 等于
        //   在"书本身是抽奖池产物"之上 再 被 随机性 罚一次 —— 玩家 可能 读烂三本 都 摸不到它。
        //   所以 被动 改走"读透"这条 确定通路：耐久 见底 那一刻 一次性 逐个 尝试学习。
        public string[] 读透被动;
    ...
    public class 技能数据
    {
        ...
        public string 被动类型;   // "增幅"（叠加 战斗属性；"机制" 预留 = 订阅 战斗事件）
