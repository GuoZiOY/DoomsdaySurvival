using System;
    ...
    public class 物品数据
    {
        ...
        // 注：原先还有 `使用目标`/`使用目标枚举`（"我方单体"/"敌方单体"）与 `指定词缀属性`/`指定词缀属性枚举`（宝石镶缀用）——
    ...
    public class 训练项目
    {
        ...
        public string 名称;
        public string 描述;
        // ⚠ 取值必须是 **体质/力量/智慧/敏捷/意志**（玩家档案 用五维末日名）。
        //    原注释写的是 "体力"/"力量"/"智力"/"敏捷" —— 那是**奇幻别名**，而两处 `解析属性()`（PlayerService / 训练逻辑）
        //    的 switch 里**根本没有"体力"/"智力"** → 会落到 default：PlayerService 兜 体质（碰巧对）、
        //    训练逻辑 兜 **力量**（错得离谱）。training.json 目前不存在，所以这个坑还没被踩到，但注释必须先改对。
        public string 属性;
        public int 加成;          // 每次 +N
        public int 花费;
    }
    ...
    //
    // 种子口径（**两把钥匙**，见 docs/大世界网格与副本设计.md §3.1）：
    //   大世界布局种子 = 派生(玩家档案.世界种子, 世界模板.标识)   → 大世界布局永远固定
    //   区域布局种子   = 派生(世界种子, "区域:" + 区域标识)        → 同一存档内那片区域永远同一套楼
    //   战局代次（随档，副本重置时 +1）**只进"内容键"，不进布局种子** —— 这样"布局固定"与"内容可刷"才能同时成立。
    ...
    public class 世界搜索参数
    {
        public int 行动点 = 10;          // 与房间层"首次搜索容器"的 10 行动点对齐（v48 定的）
