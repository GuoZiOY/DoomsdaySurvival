using System;
    ...
    //   ≤10  → 只有贴脸才听得见（潜行路线可用）
    //   25~45 → 附近一圈被惊动（翻柜子有代价，但可控）
    //   ≥80  → **全街区广播**（枪声 / 砸锁）—— "带枪的代价"第一次真实存在
    // 放在 Data 层：Domain 的 AI 与 Services 的动作点都要读；Services 不能反向给 Domain 喂常量。
    public static class 噪音强度
    {
        public const int 基准 = 50;            // 听觉半径 的基准强度
        public const int 走一格 = 5;
        public const int 搜索容器首次 = 25;
        public const int 搜索容器重复 = 10;
        public const int 撬锁 = 35;
        public const int 砸锁 = 80;
        public const int 大世界搜索 = 45;
        public const int 开枪 = 90;
        public const int 弓弩 = 30;
        public const int 近战 = 25;
        public const int 敌人倒下 = 40;
        public const int 破坏 = 70;
    }
