...
# 已知违规白名单（这 8 处 + 下面 1 处 Domain/UnityEngine = **架构债 9 处**，每处都写了"为什么先留着 + 打算怎么修"）
# 修完一处就删一行；**白名单清零那天就是 asmdef 硬化的时机**（档3）。
# 刀30 已清 3 条：Data→Domain(建筑外形 / 品质·伤病类型)、Domain→Events(变化原因) —— 见 docs §一之十六。
$白名单 = @{
...
foreach ($f in $文件) {
    ...
    $净 = [regex]::Replace($净, '(?m)//.*$', '')
    # 去**普通字符串字面量**（v51 刀46 修假阳性）：`加载("敌人特性", …)` 里的 "敌人特性" 是数据文件名，
    #   但分词器只按"标识符"切，会把字符串里的字也当成类型名 → 报一条根本不存在的越层引用。
    #   ⚠ 只去**前面不是 $ 的**引号对：插值字符串 `$"{敌人特性.名()}"` 里的**代码**是真引用，必须留着。
    #   ⚠ 字符类里**必须排除换行**：`[^"\\]` 会把换行也吃进去 → 整个文件被压成一行（第一版就踩了，行号全废）。
    $净 = [regex]::Replace($净, '(?<!\$)"[^"\r\n\\]*(?:\\.[^"\r\n\\]*)*"', '""')
    # 插值字符串 `$"…{代码}…"`：**只保留 {…} 里的代码**，去掉字面文字。
    #   为什么：`$"敌人特性[{标识}] 没有效果"` 里的"敌人特性"是**提示文本**，不是引用（第二类假阳性），
    #   而 `$"{敌人特性.名()}"` 里的**是真引用** —— 一刀切会漏报，所以按 {} 拆开只留代码。
    $净 = [regex]::Replace($净, '\$"((?:[^"\\{}]|\\.|\{[^{}]*\})*)"', {
        param($m)
        $码 = [regex]::Matches($m.Groups[1].Value, '\{([^{}]*)\}') | ForEach-Object { $_.Groups[1].Value }
        return ($码 -join ' ')
    })
    # 行号：逐行扫，方便报位置
    $行们 = $净 -split "`r?`n"
