...
public static class Program
{
    public static int Main(string[] args)
    {
        ...
        foreach (var 区模板 in 区域根.区域)
        {
            ...
            for (int 种子 = 1; 种子 <= 种子数; 种子++)
            {
                ...
                foreach (var 楼 in 楼们)
                {
                    ...
                    foreach (var 障碍 in 区.取类型(网格实体类型.障碍))
                        if (Math.Abs(障碍.列 - 门.列) <= 1 && Math.Abs(障碍.行 - 门.行) <= 1)
            ...
            汇总.Add($"区域[{区模板.标识}] {区模板.列}×{区模板.行}：平均建筑 {(double)建筑数 / 种子数:F1} / 地块 {(double)地块数 / 种子数:F1}"
                   + $" / 障碍 {(double)障碍数 / 种子数:F1}（清单 {清单.Count} 栋）");
        }
    ...
    {
        Console.WriteLine();
        Console.WriteLine($"【示例】{区.名称} {区.列}×{区.行} 建筑 {楼们.Count} 障碍 {区.取类型(网格实体类型.障碍).Count}（{模板.描述}）");
