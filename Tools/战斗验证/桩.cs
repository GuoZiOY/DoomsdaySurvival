...
public sealed class 玩家档案
{
    ...
    // 被动技能（2026-09-17）：`战斗单位.从玩家投影` 按它分流 主动/被动 + 读被动增幅。真实档案由 PlayerService
    //   接线到 DataService.技能；桩里恒 null（等价于"6 格里全是主动"）。
    //   ⚠ 真实 `玩家档案` 新增成员时，这里要同步，否则本验证器编译不过（这是**好事**：它逼桩跟上真实档案）。
    public Func<string, 技能数据> 技能解析;
