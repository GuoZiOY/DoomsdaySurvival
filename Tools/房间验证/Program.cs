...
// 校验项（任何一条挂了就是 bug）：
//   ① 确定性：同种子两次生成 → 结构指纹一致；
//   ② 结构：入口格可通行（玩家站得住）；容器/敌人不越界、不重叠、不压通道格（入口 + 门 + 门内侧）；
...
//   ⑧ 门：**外墙上真的开了洞**（门格上没有墙、可通行、不占格）、门在外墙上且朝向与所在边一致、
//         离墙角 ≥2 / 同边两门 ≥3、门内侧格可通行且可达；内门数量 = 模板声明数、含大门时正门恰一扇。
// ============================================================
public static class Program
{
    ...
    // 统计
    private static int 总房间, 保底数, 重掷总次数, 容器总, 敌人总, 坏容器总, 坏敌人总, 门总;
    ...
    public static int Main(string[] args)
    {
        ...
        Console.WriteLine($"平均每间：容器 {平均(容器总, 总房间):F1} / 敌人 {平均(敌人总, 总房间):F1} / 门 {平均(门总, 总房间):F1} / 重掷 {平均(重掷总次数, 总房间):F2} 次");
    ...
    private static void 跑一颗(测试数据 数据, 房间模板 模板, int 种子)
    {
        ...
        foreach (var 容器 in 甲.取类型(网格实体类型.容器))
            if (!有相邻可达格(甲, 可达, 容器))
                报错(模板, 种子, $"容器搜不到（没有相邻可达格）：{容器.定义标识}@{容器.列},{容器.行}");
        foreach (var 敌 in 甲.取类型(网格实体类型.敌人))
            if (!有相邻可达格(甲, 可达, 敌))
    ...
    private static void 测门(测试数据 数据, 网格数据 房, 房间模板 模板, int 种子, HashSet<int> 可达)
    {
        var 门们 = 房.门列表();
        门总 += 门们.Count;

        // 大门：默认（含大门）必须恰好一扇，且它就贴在入口格外侧（入口格 = 大门内侧格）
        var 大门 = 房.大门();
        if (大门 == null) 报错(模板, 种子, "默认生成却没有大门");
        else
        {
            var 内 = 房.门内侧格(大门);
            if (内.列 != 房.入口列 || 内.行 != 房.入口行)
                报错(模板, 种子, $"大门不在入口格外侧（入口 {房.入口列},{房.入口行} vs 大门内侧 {内.列},{内.行}）");
        }

        // 内门数量必须等于模板声明数（排不开就会少，少一扇就是"数据里写了门却走不过去"）
        int 声明内门 = 模板.门?.Length ?? 0;
        int 实际内门 = 0;
        foreach (var 门 in 门们) if (!门.是大门) 实际内门++;
        if (实际内门 != 声明内门) 报错(模板, 种子, $"内门 {实际内门} 扇 ≠ 模板声明 {声明内门} 扇");
        if (房.门开不出 != 0) 报错(模板, 种子, $"有 {房.门开不出} 扇门排不开（房间太小 / 同边门太多）");

        for (int i = 0; i < 门们.Count; i++)
        {
            var 门 = 门们[i];
            // **洞真的开了**：那格上不能还有墙、必须能走、门自己不占格（**锁着的门是例外**：
            // 它就是"一扇关着的门"——占格 + 挡视线，只能站在门前那一格，见下面的锁断言）
            var 格上 = 房.占位(门.列, 门.行);
            if (!门.锁着 && 格上 != null) 报错(模板, 种子, $"门格上还压着东西（墙上没开洞）：{格上.类型} {格上.定义标识}@{门.列},{门.行}");
            if (门.锁着 && 格上 != 门) 报错(模板, 种子, $"锁着的门没占住自己那格：{门.标识}@{门.列},{门.行}");
            if (房.可通行(门.列, 门.行) == 门.锁着)
                报错(模板, 种子, $"门的可通行性与锁态不符（锁着={门.锁着}，可通行={房.可通行(门.列, 门.行)}）：{门.标识}");
            if (门.锁着 != 门.挡视线 || (!门.锁着 && 门.挡视线))
                报错(模板, 种子, $"门的挡视线与锁态不符：{门.标识} 锁着={门.锁着} 挡视线={门.挡视线}");

            // 必须正好落在外墙上，朝向与所在边一致
            bool 在外墙 = 门.列 == 0 || 门.行 == 0 || 门.列 == 房.列 - 1 || 门.行 == 房.行 - 1;
            if (!在外墙) 报错(模板, 种子, $"门不在外墙上：{门.标识}@{门.列},{门.行}");
            int 期望边 = 门.行 == 房.行 - 1 ? 0 : (门.列 == 0 ? 1 : (门.行 == 0 ? 2 : 3));
            if (门.朝向 != 期望边) 报错(模板, 种子, $"门朝向 {门.朝向} 与所在边不符（应为 {期望边}）：{门.标识}");

            // 离墙角 ≥2、内侧格能走且从入口可达
            bool 竖边 = 门.朝向 == 0 || 门.朝向 == 2;
            int 号 = 竖边 ? 门.列 : 门.行;
            int 长 = 竖边 ? 房.列 : 房.行;
            if (号 < 房间生成器.门离角 || 号 > 长 - 1 - 房间生成器.门离角)
                报错(模板, 种子, $"门贴墙角：{门.标识} 第{号}格（边长 {长}）");
            var 内侧 = 房.门内侧格(门);
            if (!房.可通行(内侧.列, 内侧.行)) 报错(模板, 种子, $"门内侧格不可通行：{门.标识} → {内侧.列},{内侧.行}");
            if (!可达.Contains(网格数据.编码(内侧.列, 内侧.行)))
                报错(模板, 种子, $"门内侧格从入口走不到：{门.标识} → {内侧.列},{内侧.行}");

            // 门锁：锁着 → 恰有钥匙标识（而且这间房得有柜子放钥匙）；无锁 → 钥匙标识为空
            if (!门.是大门)
            {
                string 声明的锁 = "";
                if (模板.门 != null)
                    foreach (var 项 in 模板.门)
                        if (项 != null && 项.通向 == 门.通向 && 项.边 == 边名(门.朝向)) 声明的锁 = 项.锁 ?? "";
                if ((门.锁钥匙 ?? "") != 声明的锁)
                    报错(模板, 种子, $"门的锁与模板声明不符：{门.标识} 实际[{门.锁钥匙}] 声明[{声明的锁}]");
                if (!string.IsNullOrEmpty(门.锁钥匙) && 房.容器总数() == 0 && 房.敌人总数() == 0)
                    报错(模板, 种子, $"门锁着但房间既没容器也没敌人（钥匙无处可放）：{门.标识}");
            }

            // 内门必须写明通向谁，且那个模板真的存在（大门通向外面 = 空，合法）
            if (!门.是大门)
            {
                if (string.IsNullOrEmpty(门.通向)) 报错(模板, 种子, $"内门没写通向：{门.标识}");
                else if (!数据.房间模板.ContainsKey(门.通向)) 报错(模板, 种子, $"门通向的房间模板不存在：{门.通向}");
            }

            // 同一条边上两扇门 ≥3 格（且不能同一格）
            for (int j = i + 1; j < 门们.Count; j++)
            {
                if (门们[j].朝向 != 门.朝向) continue;
                int 他号 = 竖边 ? 门们[j].列 : 门们[j].行;
                if (Math.Abs(他号 - 号) < 房间生成器.门间距)
                    报错(模板, 种子, $"同边两扇门太近：{门.标识} 第{号}格 与 {门们[j].标识} 第{他号}格");
            }
        }

        // 含大门 = false（从门进来的房间）：大门必须没有，内门照旧
        var 无大门 = 房间生成器.生成(种子, 模板, 数据.取容器表, 数据.取敌人组, false);
        if (无大门?.大门() != null) 报错(模板, 种子, "含大门=false 却生成了大门");
        if (无大门 != null)
        {
            int 甲内门 = 0;
            foreach (var 门 in 无大门.门列表()) if (!门.是大门) 甲内门++;
            if (甲内门 != 声明内门) 报错(模板, 种子, $"含大门=false 时内门 {甲内门} 扇 ≠ 模板声明 {声明内门} 扇");
        }
    }

    // 朝向码 → 边名（与 网格实体.朝向 同义：0下 1左 2上 3右）
    private static string 边名(int 朝向) => 朝向 == 0 ? "下" : 朝向 == 1 ? "左" : 朝向 == 2 ? "上" : "右";

    private static void 测寻路(网格数据 房, 房间模板 模板, int 种子)    {
        ...
        // 点到容器 = 走到相邻格
        foreach (var 容器 in 房.取类型(网格实体类型.容器))
        {
            var 路 = 网格寻路.寻路到相邻(房, (房.入口列, 房.入口行), 容器);
        ...
        var 墙 = 房.取类型(网格实体类型.墙);
        if (墙.Count > 0)
        {
            var 路 = 网格寻路.寻路(房, (房.入口列, 房.入口行), (墙[0].列, 墙[0].行));
            if (路 != null) 报错(模板, 种子, "寻路到墙上却没返回 null");
        }
    }

    private static void 测视野(网格数据 房, 房间模板 模板, int 种子)
    {
        var 可见 = 网格视野.可见集(房, 房.入口列, 房.入口行, 房.视野边长);
        if (可见.Count == 0) 报错(模板, 种子, "入口处可见集为空");
        if (!可见.Contains(网格数据.编码(房.入口列, 房.入口行))) 报错(模板, 种子, "可见集不含玩家自己所在格");
    ...
    private static bool 有相邻可达格(网格数据 房, HashSet<int> 可达, 网格实体 e)
    {
        for (int r = e.行; r < e.行 + e.高; r++)
            ...
            {
                if (房.界内(c - 1, r) && 可达.Contains(网格数据.编码(c - 1, r))) return true;
                if (房.界内(c + 1, r) && 可达.Contains(网格数据.编码(c + 1, r))) return true;
                if (房.界内(c, r - 1) && 可达.Contains(网格数据.编码(c, r - 1))) return true;
                if (房.界内(c, r + 1) && 可达.Contains(网格数据.编码(c, r + 1))) return true;
            }
        return false;
    }
    ...
    private static void 打印示例(网格数据 房)
    {
        Console.WriteLine();
            ...
            for (int c = 0; c < 房.列; c++)
            {
                ...
                if (格 != null)
                {
                    行文本.Append(格.类型 switch
                    {
                        网格实体类型.墙 => '#',
                        网格实体类型.容器 => 格.挡视线 ? 'B' : 'c',
                        网格实体类型.门 => 格.是大门 ? (格.锁着 ? 'L' : 'D') : (格.锁着 ? 'l' : '+'),
                        网格实体类型.尸体 => 'x',
                        网格实体类型.敌人 => '!',
                        _ => '?',
                    });
                ...
                行文本.Append(可见.Contains(网格数据.编码(c, r)) ? '·' : ' ');
            }
