...
$历史快照 = @{
    '目录调整对照表.md'         = '旧 → 新 的对照表：旧路径本来就**不该**存在，存在了才是错的'
    '房间层-第1刀设计方案.md'   = '当刀设计快照：里面点名的 房间数据/房间寻路/房间视野/房间种子 后来被合并或删掉了'
    '房间层-第1刀实现说明.md'   = '当刀实现快照，同上'
}
# 目录级豁免
$目录豁免 = @{ '评估' = '六份历史审计报告（01~06）：按约定不改写，它们记录的是审计当时的现状' }
