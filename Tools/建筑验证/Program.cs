        ...
        foreach (var 建 in 建筑根.建筑)
        {
            ...
            for (int 种子 = 1; 种子 <= 种子数; 种子++)
            {
                ...
                for (int 层 = 0; 层 < 层数; 层++)
                {
                    ...
                    // ③'' 楼梯那一列/行（外线）**只允许 楼梯 / 墙 / 界外**，别的一律是 bug。
                    //      v51 刀15 用户报的 bug：门有概率生成在楼梯那一列 → 楼梯上不了、门也进不去。
                    //      根因是 `造空房` 的顺序（加楼梯间 → 开门）而 `开门` 用的是 内缩 0（正好是凸出那一行/列）。
                    //      ⚠ 这里必须**遍历实体**而不是 `格上实体()`：后者只返回第一格实体，
                    //        而"门压在楼梯格上"正是"一格两个实体"，用格上实体查会漏掉后加的那个。
                    {
                        bool 竖线 = 建.楼梯边 == "左" || 建.楼梯边 == "右";
                        foreach (var e in 房.实体)
                        {
                            if (e == null) continue;
                            bool 在线 = 竖线 ? e.列 == 梯.列 : e.行 == 梯.行;
                            if (!在线) continue;
                            if (e.类型 == 网格实体类型.楼梯 || e.类型 == 网格实体类型.墙
                                || e.类型 == 网格实体类型.界外) continue;
                            失败++;
                            Console.WriteLine($"✗ 建筑[{建.标识}] 种子{种子} 层{层 + 1}：楼梯那一"
                                            + $"{(竖线 ? "列" : "行")}({e.列},{e.行}) 上有 {e.类型}[{e.标识}]"
                                            + " —— 那一条线只该有 楼梯/墙/界外（门会压在楼梯上或掉进界外，交互不了）");
                        }
                    }
