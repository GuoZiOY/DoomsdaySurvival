...
# ---------- 4. 路径池必须"从终点往前"分配 + 防拖尾偏移 ----------
# 为什么钉：走路时路径每步从队首缩短一格。按"从头数"分配会让几乎每张路径图都改画到另一格
#           （离线实测：一条 30 格直线走完，平均每步 16 张 → 从尾分配 1 张）。
# 刀22 又加了一条：令牌**开始走**那一刻就要把刚离开那一格的高亮收掉（用户反馈"看着像拖尾"）→
#   刷路径 从 `首 = 路径起点偏移` 开始画，推进移动 在开段时把它置 1、踩到格心后归 0。
Write-Output "[4] 刷路径 必须从终点往前分配池槽（每步只动队尾）+ 队首偏移防拖尾"
$刷路径体 = 取函数体 $图层 (找行 $图层 'private\s+void\s+刷路径\s*\(')
if ($null -eq $刷路径体) { 报错 "取不到 刷路径() 的函数体" }
else {
    if (-not (体内有 $刷路径体 'for\s*\(int\s+i\s*=\s*路\.Count\s*-\s*1;\s*i\s*>=')) { 报错 "刷路径 不再「从终点往前」分配（顺序又倒回去了？）" }
    if (体内有 $刷路径体 'for\s*\(int\s+i\s*=\s*0;\s*i\s*<\s*路\.Count\s*&&') { 报错 "刷路径 里又出现了「从头数」的分配循环（i = 0; i < 路.Count && 用 < 路径池.Count）" }
    if (-not (体内有 $刷路径体 '路径起点偏移')) { 报错 "刷路径 没认 路径起点偏移 —— 高亮又会拖在角色后面（用户反馈过的那个）" }
}
$推进体 = 取函数体 $图层 (找行 $图层 'private\s+void\s+推进移动\s*\(')
if ($null -eq $推进体) { 报错 "取不到 推进移动() 的函数体" }
else {
    if (-not (体内有 $推进体 '路径起点偏移\s*=\s*1')) { 报错 "推进移动 开段时没把 路径起点偏移 置 1 —— 拖尾会回来" }
    if (-not (体内有 $推进体 '路径起点偏移\s*=\s*0')) { 报错 "推进移动 踩到格心后没把 路径起点偏移 归 0 —— 脚下那一格会一直不亮" }
}
...
$渲染 = 读行 (Join-Path $根 "Assets\Scripts\UI\网格\网格面板基类.渲染.cs")
