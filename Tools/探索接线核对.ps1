...
# ④ 即时制接线：大世界敌人的行动必须挂在**游戏时间**上，不能挂在"玩家走一步"上。
#   为什么钉（刀61 用户纠正）：我第一版把驱动写在 玩家走了一步() 里 → **站着不动世界就冻结**。
#   那是把即时制当回合制写，而且从录屏上看像"AI 根本没做"，极容易漏。
#   本作时间由 世界时间管理器 每帧推进（净化/生长/阅读 都是这个模式），敌人必须同样挂上去。
$时间器 = Join-Path $根 "Assets\Scripts\Services\世界\世界时间管理器.cs"
$时间器行 = 去注释 (读行 $时间器)
foreach ($钩 in @('推进', '跳时')) {
    $体 = 取函数体 $时间器行 (找行 $时间器行 ("public\s+void\s+" + $钩 + "\s*\("))
    if ($null -eq $体) { 报错 ("取不到 世界时间管理器.{0}() 的函数体" -f $钩) }
    elseif (-not (体内有 $体 '大世界探索服务[^\r\n]*时间推进')) {
        报错 ("世界时间管理器.{0}() 没有把游戏时间喂给 大世界探索服务.时间推进 —— 敌人会退回'只在玩家走一步时动'（即时制变回合制）" -f $钩)
    }
}
# ⑤ 反向断言：不许再把敌人驱动塞回"玩家走一步"（那是同一个 bug 的入口）
$大世界去 = 去注释 (读行 $大世界服)
$走一步体 = 取函数体 $大世界去 (找行 $大世界去 'override\s+void\s+玩家走了一步\s*\(')
if ($null -ne $走一步体 -and (体内有 $走一步体 '驱动敌人')) {
    报错 "大世界探索服务.玩家走了一步() 里又在驱动敌人 —— 那会让'站着不动世界冻结'（即时制退回回合制）"
}
