...
# ③ 噪音结算必须是**基类的具体实现**（刀61 第 5 次修正：三层共用，不再由大世界一家覆写）
$格子服 = Join-Path $根 "Assets\Scripts\Services\探索\格子探索服务.cs"
$格子行 = 去注释 (读行 $格子服)
if ((找行 $格子行 'protected\s+void\s+噪音结算\s*\(\s*噪音事件') -lt 0) {
    报错 "格子探索服务 里没有具体的 噪音结算(噪音事件) —— 三层共用一份实现，不能只挂在大世界"
}
# ④ 敌人 AI 必须在**基类**（否则"只有大世界会动"那个坑会重新长出来：玩家在建筑里看敌人纹丝不动）
if ((找行 $格子行 'public\s+void\s+时间推进\s*\(\s*float') -lt 0) { 报错 "格子探索服务 里没有 时间推进(float) —— 三层共用入口应该在基类" }
if ((找行 $格子行 'private\s+void\s+驱动敌人\s*\(\s*int') -lt 0) { 报错 "格子探索服务 里没有 驱动敌人(int 整数分) —— AI 驱动应该在基类" }
# ⑤ 即时制接线：世界时间管理器 的**两条时间流**都必须喂**三层**
$时间器 = Join-Path $根 "Assets\Scripts\Services\世界\世界时间管理器.cs"
$时间器行 = 去注释 (读行 $时间器)
foreach ($钩 in @('推进', '跳时')) {
    $体 = 取函数体 $时间器行 (找行 $时间器行 ("public\s+void\s+" + $钩 + "\s*\("))
    if ($null -eq $体) { 报错 ("取不到 世界时间管理器.{0}() 的函数体" -f $钩); continue }
    foreach ($层 in @('大世界探索服务', '区域探索服务', '房间探索服务')) {
        if (-not (体内有 $体 ($层 + '[^\r\n]*时间推进'))) {
            报错 ("世界时间管理器.{0}() 没有喂 {1}.时间推进 —— 那一层的敌人会退化成静态（玩家在那儿看到的就是'敌人没动过'）" -f $钩, $层)
        }
    }
}
# ⑥ 三层都必须给"撤退点"：会动的敌人是活的墙，不给撤退点 = 基类默认不校验 = 可能把玩家围死
foreach ($对 in @(@('大世界探索服务', '大世界探索服务.cs'), @('区域探索服务', '区域探索服务.cs'), @('房间探索服务', '房间探索服务.cs'))) {
    $f = Join-Path (Join-Path $根 "Assets\Scripts\Services\探索") $对[1]
    if (-not ((去注释 (读行 $f)) -match 'override\s+void\s+收撤退点')) {
        报错 ("{0} 没有覆写 收撤退点 —— 这一层的敌人移动后不校验'玩家还走得出去吗'（可能被围死）" -f $对[0])
    }
}
# ⑦ 反向断言：不许再把敌人驱动塞回"玩家走一步"（那是"站着不动世界冻结"的入口）
$走一步体 = 取函数体 $格子行 (找行 $格子行 'protected\s+virtual\s+void\s+玩家走了一步\s*\(')
if ($null -ne $走一步体 -and (体内有 $走一步体 '驱动敌人')) {
    报错 "格子探索服务.玩家走了一步() 里又在驱动敌人 —— 那会让'站着不动世界冻结'（即时制退回回合制）"
}
# ⑥ 实体移动的渲染接线（刀61）：**实体会挪格了**（大世界敌人巡街）——
#   这是本项目第一次出现"除玩家之外会动的实体"。渲染**不能只靠显示事件**：
#   `大世界面板.刷新网格()` 第一行是 `if (!gameObject.activeInHierarchy) return;`，
#   面板那一刻不激活（战斗中 / 切到别的面板）这次刷新就被丢掉，而服务侧敌人已经挪过去了
#   → 令牌永远停在旧格，**还不报任何错**（表现就是"敌人根本没动过"）。
#   所以要求 探索图层 有一个按"实体位置签名"去重的重摆钩子，与 刷迷雾/刷路径 同一个套路。
Write-Output "[12] 实体移动的渲染接线（位置签名去重 → 面板重摆实体框）"
$刷实体体 = 取函数体 $图层 (找行 $图层 'private\s+void\s+刷实体\s*\(')
if ($null -eq $刷实体体) { 报错 "探索图层 里没有 刷实体() —— 实体会动之后渲染只剩'显示事件'一条路（面板不激活时会漏刷）" }
elseif (-not (体内有 $刷实体体 '请求刷新')) { 报错 "刷实体() 没有调用 面板.请求刷新()" }
$图层更新体 = 取函数体 $图层 (找行 $图层 'private\s+void\s+Update\s*\(')
if ($null -eq $图层更新体) { 报错 "取不到 探索图层.Update() 的函数体" }
elseif (-not (体内有 $图层更新体 '刷实体\s*\(\s*\)')) { 报错 "探索图层.Update() 没有调用 刷实体() —— 那个钩子等于没接" }
$图层重建体 = 取函数体 $图层 (找行 $图层 'private\s+void\s+重建\s*\(')
if ($null -ne $图层重建体 -and -not (体内有 $图层重建体 '上次实体签名\s*=\s*int\.MinValue')) {
    报错 "重建() 没有重置 上次实体签名 —— 重建后第一次位置变化会被去重键吃掉（实体框停在旧格）"
}

Write-Output ""
if ($script:失败 -eq 0) {
    Write-Output "✅ 探索接线核对通过（雾输入写入点 / 去重键重置 / 池重排强制刷雾 / 路径从尾分配 / CanvasGroup 缓存 / 底格与拖拽开关 / 单面交互层 / 平铺格线 / 实体框骨架在基类 / 噪音接线 / 即时制时间接线 / 实体移动渲染接线）"
